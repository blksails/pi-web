...
- [ ] 1. 协议:surface 命令 payload/result schema
- [ ] 1.1 定义 `SurfaceCommandPayloadSchema` / `SurfaceCommandResultSchema` + `SurfaceKey` 类型 (P)
...
- [ ] 2. agent 侧:进程内 surface 注册表 seam
- [ ] 2.1 实现 `surfaceRegistry` + `__piWebSurfaces__` seam
...
- [ ] 3. agent 侧:createSurface 门面
- [ ] 3.1 实现 `createSurface(pi, config)`
...
- [ ] 4. runner 侧:ui-rpc 命令接收与派发接线
- [ ] 4.1 实现 `wireSurfaceBridge`
...
- [ ] 5. UI 侧:useSurface hook
- [ ] 5.1 实现 `useSurface(domain)`
...
- [ ] 6. 领域无关示例 surface(e2e/集成夹具)
- [ ] 6.1 实现 `surface-demo-agent`
  ...
  - _Depends: 3.1, 5.1_
- [ ] 6.2 注册示例到 webext-registry(e2e 静态加载)
...
- [ ] 9. 质量门与宿主中立性验收
- [ ] 9.1 全量 typecheck + 受影响包测试 + 中立性 grep
