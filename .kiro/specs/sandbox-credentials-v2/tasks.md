...
- 4.1 完成:`e2e/aigc-proxy/{proxy-chain.local.mjs,server-entry.ts,sandbox-child.ts}` 改造为 `e2e/llm-gateway/{gateway-chain.local.mjs,server-entry.ts,sandbox-child.ts}` 后删除原目录;`package.json` 的 `e2e:aigc-proxy` 脚本改名为 `e2e:llm-gateway`。三进程编排骨架(spawn/stub/断言)完整保留,`server-entry.ts` 改用真实 `createLlmGatewayRoutes`(自定义 `registry.newapi.upstreamBase` 指向本地 stub,而非内置表/JSON 覆盖)+ 真实 `mintScopedToken`/`resolveLlmGatewaySecret`,并复用真实 `buildSandboxLlmEnv`(`lib/app/llm-gateway-config.ts`,相对路径 import,验证其产物跨仓契约键名);`sandbox-child.ts` env 仅含 `PI_LLM_GATEWAY_BASE`/`PI_LLM_TOKEN_NEWAPI`,自检 `PROVIDER_KEY_NAMES` 镜像列表(该常量未在 `lib/app/config.ts` 导出,故与已摘除的 aigc-proxy e2e 同惯例镜像一份字面量)全程不泄露;主对话请求用**有状态 SSE 增量分帧器**(逐次消费完整 `\n\n` 事件、残余留到下次拼接——首版用无状态"每次 read 都对累积 buffer 全量 re-split"踩坑导致同一事件被重复计入,已修正)解析流式响应,拼接 `delta.content` 与 stub fixture 逐字精确比较,并用多次独立 `read()` 之间的到达时间间隔(`spanMs >= 40`)证明分块非整体缓冲。`gateway-chain.local.mjs` 内直接 `fetch` 编排侧断言无 token/过期 token/错 scope token 三种负路径(401/401/403)+ `/api/aigc-proxy/*` 404(证 6.4),`server-entry.ts` 的 `sse.basePath` 与真实装配(`lib/app/pi-handler.ts`)一致为 `/api`,使 404 断言天然对齐生产路由前缀。`pnpm e2e:llm-gateway` 连跑三次 exit 0 全绿(11 项断言,无 flake);全仓 `npx tsc --noEmit` 归零(原 2 个 `e2e/aigc-proxy/server-entry.ts` 错误随目录删除消失,新文件仅踩过一次 `noUncheckedIndexedAccess` 严格空值检查已修);`grep -rn "aigc-proxy" e2e package.json` 仅剩注释/字面测试路径(说明改造历史 + `/api/aigc-proxy/*` 本就是被测的 404 目标路径),`package.json` 无 `e2e:aigc-proxy` 残留脚本。未触碰 3.1-3.4 已完成的产品代码(网关路由/装配/tokens 全部只读引用)。
- 4.3 完成(纯文档,零代码改动):新增 `docs/sandbox-credentials-attachment-deployment.md`,覆盖推荐形态(`cloud-http` 回环,`X-Pi-Attachment-Token`,拓扑 JSON 示例)/显式选项(`s3` 直连 `accessKeyEnv`/`secretKeyEnv`,凭据经 `computePassthroughEnv` 进沙箱的暴露面说明)/LLM 网关迁移警示(NEWAPI/SUFY/DASHSCOPE 同时是 AIGC 三键,平台注入 vs `PI_WEB_E2B_ENV_PASSTHROUGH` 显式透传两条路径)三点。**核对现状代码时发现 design.md §Security Considerations 的表述与实现有出入**:design 写「装配期对此情形(网关启用且无 AIGC 覆盖注入)输出可识别提示日志」,但 `lib/app/llm-gateway-assembly.ts`(3.3 已实现)的 `warn` 字段只在**网关未配置**分支产出,**网关已配置**分支不区分「AIGC 三键是否已被其他路径补齐」、不输出对应 warn——文档已按实际实现措辞(不宣称不存在的日志行为),后续任务(如 4.4 真沙箱手动验证)如需依赖该 warn 日志做证据,应先确认这一差异,不要假设 design.md 原文的日志行为已落地。