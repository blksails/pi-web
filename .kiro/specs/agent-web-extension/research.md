...
- **Key Findings**:
  ...
  - 现有 transport 已有版本化 SSE 帧(`uiMessageChunk` / `control`)与 REST DTO(`@blksails/protocol`)。Tier 3 的 UI↔agent RPC 应**复用 control 帧(下行)+ REST 命令(上行)**,新增 `ui-rpc` 控制载荷与 `/sessions/:id/ui-rpc` 端点,而非另起传输。
...
- **Findings**: 浏览器原生 ESM + import map 可把裸 specifier(`react`/`react-dom`/`@blksails/web-kit`/设计系统)映射到宿主已加载的单例 URL;`pi-web build` 须把这些标 external 并在产物校验阶段拒绝内联副本。
