...
- **In scope**:
  ...
  - `@blksails/pi-web-kit` 包(`defineWebExtension` + RPC client + 复用组件 + 类型)与 `pi-web build` 工具(强制 externals、CSS scoping、剥全局样式、产出 manifest + SRI)。
...
- **Adjacent expectations**:
  - 宿主既有 `@blksails/pi-web-protocol` 契约为 RPC/传输根;本特性新增的 UI↔agent 消息须沿用其版本化与校验约定。
...
6. The UI↔agent RPC 消息 shall 携带协议版本,且与 `@blksails/pi-web-protocol` 的校验约定一致。
...
#### Acceptance Criteria
1. The WebExtension 产物 shall 为自包含 ESM bundle,且将 `react`、`react-dom`、`@blksails/pi-web-kit`、设计系统标记为 external。
...
### Requirement 9: `@blksails/pi-web-kit` 包与 `pi-web build` 工具

**Objective:** 作为 agent source 作者,我想用一个独立的作者侧 SDK 与构建命令编写并打包 `.pi/web`,以便有稳定、类型安全的扩展编写契约。

#### Acceptance Criteria
1. The `@blksails/pi-web-kit` 包 shall 导出 `defineWebExtension()`、回 agent 的 RPC client、可复用设计原语/组件、类型定义与 `targetApiVersion`。
2. The `@blksails/pi-web-kit` 公共 API shall 遵循语义化版本,并标注稳定核与 experimental 区。
3. The `pi-web build` 工具 shall 强制 externals(打入自有 React/web-kit 副本则拒绝出包)、执行 CSS scoping(见 Requirement 8)、产出 manifest 与 SRI。
4. The `@blksails/pi-web-kit` shall 与后端解耦,仅经 `@blksails/pi-web-protocol` 契约/RPC 与 agent 通信,不依赖 server 内部实现。
5. The `@blksails/pi-web-kit` 的扩展定义入口 shall 与现有 `@blksails/pi-web-agent-kit` 的 `defineAgent()` 在使用范式上保持对称。
