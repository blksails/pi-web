...
**Impact**: 把 `PiChat` 既有四维定制(slots/components/registry/presets)的「喂入源」从宿主装配点的 React props 迁移为 agent source 声明 + 运行时加载;新增 `@blksails/pi-web-kit` 包与 `pi-web build` 工具;registry 从模块级单例改为 per-session 作用域;在既有 `@blksails/pi-web-protocol` 通道上新增 UI↔agent RPC 子协议。
...
### This Spec Owns
- `.pi/web` 目录契约、`WebExtensionManifest` 与 `WebExtension` 描述符(`@blksails/pi-web-protocol` + `@blksails/pi-web-kit`)。
...
- `@blksails/pi-web-kit` 包与 `pi-web build` 工具(externals 强制、CSS scoping、剥全局样式、产出 manifest+SRI)。
...
- `@blksails/pi-web-protocol`(契约根;新增 manifest/ui-rpc schema 于此)。
- `@blksails/pi-web-react`(transport/hooks)、`@blksails/pi-web-ui`(PiChat 宿主)。
...
- `@blksails/pi-web-kit` 公共 API 主版本变更 → 全量示例 bundle 重构建 + CI 冒烟。
- registry 作用域/优先级语义变更 → 既有 PiChat 宿主装配路径。

## Architecture

### Existing Architecture Analysis
- 分层:`protocol ← server`、`protocol ← react ← ui`、app 注入装配。SSE 帧分 `uiMessageChunk`/`control` 两类且带 `protocolVersion`;REST DTO 在 `@blksails/pi-web-protocol`。
...
| Author SDK | `@blksails/pi-web-kit`(新, workspace) | `defineWebExtension` + RPC client + 复用组件 + 类型 | 与 `@blksails/pi-web-agent-kit` 对称 |
| Build | `esbuild` ^0.24 + 自研 CSS scoping 插件 | `pi-web build`:externals/scoping/manifest/SRI | 随 web-kit 发布 |
| Protocol | `@blksails/pi-web-protocol`(扩展)+ zod | manifest schema、ui-rpc schema、artifact 消息 schema | 带 protocolVersion |
| Backend | `@blksails/pi-web-server`(扩展 http)| `/sessions/:id/ui-rpc` 端点 + control 帧 `ui-rpc` 载荷转发 | runtime nodejs |
...
│       └── index.ts                     # barrel
├── web-kit/                             # 新增包 @blksails/pi-web-kit(作者侧 SDK + build)
...
### Protocol — 契约(`@blksails/pi-web-protocol/web-ext`)
...
### Author SDK — `@blksails/pi-web-kit`（Req 9）
```typescript
...
- `externals-guard`: 产物 AST/字符串扫描,若内联了 `react`/`react-dom`/`@blksails/pi-web-kit` 则 `exit 1`。
...
- `extension-loader`(Req 6): 注入 import map(`react`/`react-dom`/`@blksails/pi-web-kit`/设计系统 → 宿主单例 URL)→ `import(entryUrl)` → 返回 `WebExtension`;失败回退默认 UI。
...
## Data Models
- **WebExtensionManifest / WebExtension / UiRpc* / ArtifactMessage**: 见上,均 zod schema 推导类型,置于 `@blksails/pi-web-protocol/web-ext`,带 `protocolVersion`。
