...
- [x] 1.1 定义 `Attachment` 描述符与上传响应 DTO(协议层)
  ...
  - 观察完成:协议包通过 typecheck,可从 `@blksails/pi-web-protocol` 导入 `AttachmentSchema`/`Attachment` 与上传响应类型,schema 对缺字段/负 size 校验失败。
...
- [x] 3.3 暴露注入路由工厂并从协议/服务包导出
  ...
  - 观察完成:可从 `@blksails/pi-web-server` 导入 `createAttachmentRoutes`、store 配置工厂与上述复用面类型/类,返回的路由可直接放入 `createPiWebHandler({ routes })`。
...
- [x] 4.3 (P) 附件 UI 改用展示 URL 并呈现上传状态
...
- [x] 4.5 (P) 历史回显改为按引用走分发 URL(保留遗留 base64)
...
- [x] 6.2 e2e/集成:历史回显 URL 化与遗留 base64 防回归 + 上传失败路径
