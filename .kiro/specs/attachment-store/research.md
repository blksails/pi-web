...
- **Implications**:
  - `useAttachments` 需接受一个 `upload(file)→Promise<Attachment>` 注入(由 `@blksails/react` 客户端提供,指向 `POST /sessions/:id/attachments`),`add()` 改为异步上传、置 `uploading` 态、成功后存 `att_<nanoid>` + 展示 URL(Req 5.1/5.4/5.5)。
