...
- [x] 1.2 实现最小化预设常量与一行启用工厂并从入口导出
  ...
  - 从包入口 re-export 两个产物
  - 完成判据:`import { minimalAgentPreset, defineMinimalAgent } from "@blksails/agent-kit"` 可解析,预设对象三处关闭字段均存在
...
- [x] 1.3 为预设与工厂补充单元测试
  ...
  - 完成判据:`pnpm --filter @blksails/agent-kit test` 新增用例全绿(新鲜运行输出)
...
- [x] 2.3 为映射逻辑补充集成测试
  ...
  - 完成判据:`pnpm --filter @blksails/server test`(映射相关)新增用例全绿(新鲜运行输出)
