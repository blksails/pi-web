...
- **判据 3（点击可交互）✗ FAIL（复测 2026-08-05 深夜）**：曾短暂据用户一句
  「有反应」判过 ✓ 并收口——**判早了，撤回**。复测机械读数：对「+」两次
  聚焦后点击（落点经放大核验在按钮上、窗口有焦点）零响应零日志事件；「⟳」
  同样无响应；用户独立复现「+ 号无法点击」。三路证据一致：**chrome 可见但
  按钮不可点**。

### 判定：方案 A 部分有效——修绘制，不修交互

按本节写死的规则（三条都过才判有效）：**不判「A 有效」**。修正后的结论：

- 绘制死区已修（判据 1/2）：force_host_redraw 使 chrome 带内容可见——此前恒空白。
- **点击死区仍在**（判据 3）：事件送不进 chrome 带的按钮。此前的 hitTest 探针
  命中宿主（静态 NSView hitTest），但真实事件路由未达宿主 DOM——静态 hitTest
  证明不了事件送达，两者的判别在下一阶段做。

**方法教训（第三次栽在同一处）**：判据读数不能收「有反应」这种未指名元素与
预期效果的口头答复；「谁点的」不重要，「点了什么、预期什么、看到了什么」缺一不可。

**验证轮顺带发现（未立案的活缺陷）**：①主窗口自漂移/自增长（run1 高度逐像素
缩、run2 宽度 1200→1756 逐像素长、位置多次跳变，落位日志可复查）；②第二扇
主窗口完全不在原生布局管理下（无任何落位日志，pane 区无 chrome 带）。
