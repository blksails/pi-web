...
- **判据 3（点击可交互）——未取得干净读数**：验证时用户正在本机实际操作
  （输入法候选条持续更新、窗口多次被移动/抢焦点），自动化点击无法稳定送达
  chrome 带。**不判 pass 也不判 fail**，待机器空闲时补一次点击读数。

**归因保留**：本轮 chrome 从「恒空白」变为「可见」，与方案 A 同轮出现；
但方案 A 无独立开关，无法在同一构建内做 A/B 判别。若需严格归因，
须构建一个去掉 force_host_redraw 的对照包复跑。现状记为
「方案 A 在位时症状消失（2/3 判据过，第 3 条待补）」，不得升格为「A 已证明有效」。
