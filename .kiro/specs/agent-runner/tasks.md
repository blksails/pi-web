...
- [ ] 1. 基础:包脚手架与测试设施
- [ ] 1.1 创建 `@pi-web/agent-kit` 包脚手架与 TypeScript strict 配置
...
- [ ] 1.2 建立 `lib/pi/` 与 vitest 测试设施
  - 配置 vitest,使单元/集成/e2e 测试可由单一命令运行
  - 安装运行时依赖(pi SDK、jiti),测试依赖(`@pi-web/protocol`、vitest)
  - 观察完成:`pnpm test` 在空测试集上成功运行并报告 0 失败
  - _Requirements: 7.4_

- [ ] 2. agent-kit 类型层
- [ ] 2.1 定义 `AgentDefinition` 与 `AgentContext` 类型
...
- [ ] 2.2 实现 `defineAgent()` 恒等帮助函数并导出聚合入口
...
- [ ] 3. 选项映射与信任接线
- [ ] 3.1 (P) 实现 AgentDefinition → SDK 选项映射并组装 runtime factory
...
- [ ] 3.2 (P) 实现 `resolveProjectTrust` 接线
...
- [ ] 4. agent-loader 归一化
- [ ] 4.1 实现 jiti import 与 default export 三形态归一化
...
- [ ] 5. runner 子进程入口(集成)
- [ ] 5.1 实现 runner 参数解析、运行时组装与 runRpcMode 启动
...
- [ ] 5.2 创建示例 `examples/hello-agent/index.ts`
...
- [ ] 7. 测试:集成与 e2e(硬性)
- [ ] 7.1 集成测试:对 hello-agent 启动 runner 完成一次 prompt
...
- [ ] 7.2 e2e 测试:stdin prompt → stdout 帧 + protocol schema 校验
  - 向 runner stdin 写入 `{"type":"prompt"}`,收集 stdout 帧,断言出现 `message_update`(`text_delta`)与 `agent_end`;每帧用 `@pi-web/protocol` 的 `AgentEvent`/`RpcResponse` schema `safeParse` 全部通过(即与 CLI `pi --mode rpc` 同形)
