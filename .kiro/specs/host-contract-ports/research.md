...
- **Windows 设备名与 NTFS ADS 语法当前被键校验放行** —— `CON`、`NUL`、`file.json:stream:$DATA` 等形态不构成 EARS 1.1–1.6 所定义的路径穿越（JS 字符串与 `path.join` 不会将其解析为 `..`），故键校验层不拦。但 **`LocalWorkspace`（任务 4.x）在 Windows 上落盘时**，这些名字会命中 OS 层的特殊语义。缓解：4.1 实现键→路径映射时须显式处理；跨平台真机验证前不得宣称 Windows 可用。
