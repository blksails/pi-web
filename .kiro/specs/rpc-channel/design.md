...
- 暴露与包 `RpcClient` 对齐的命令方法封装,类型取自 `@blksails/pi-web-protocol`。
...
- `PiRpcProcess` 本地实现:按给定 spawnSpec(`SpawnSpec { cmd, args, cwd, env }`,类型自 `@blksails/pi-web-protocol` 导入)以 `detached:false` spawn 子进程并接管 stdio。
...
- **运行时**:`@blksails/pi-web-protocol`(协议类型 + `SpawnSpec`,唯一跨包依赖;`SpawnSpec` 由 protocol-contract 拥有并导出,本特性经 `import type { SpawnSpec } from "@blksails/pi-web-protocol"` 消费,不本地定义);Node 内置 `child_process`、`node:events`、`node:stream`(仅 `local` 实现内部使用,不出现在接口签名)。
...
| Infrastructure / Runtime | `child_process.spawn`(`detached:false`);`@blksails/pi-web-protocol`(类型);`vitest`(测试);`@earendil-works/pi-coding-agent`(dev/集成 spawn 真实 pi) | 子进程托管与协议类型 | pi 包不进运行时依赖 |
...
```
lib/pi/
├── pi-rpc-channel.ts         # PiRpcChannel 端口接口 + ChannelHealth 类型(SpawnSpec 自 @blksails/pi-web-protocol 导入,不在此定义)
...
- 无(greenfield 新模块)。若 monorepo 已存在 `package.json`,需将 `@blksails/pi-web-protocol` 与 `vitest` 纳入依赖——该接线随仓库初始化处理,本特性创建模块自身文件与测试。
...
| 2.1 | 按 spawnSpec spawn 接 stdio | pi-rpc-process.ts | `SpawnSpec`(自 @blksails/pi-web-protocol) | 生命周期流 |
| 2.2 | detached:false 连带清理 | pi-rpc-process.ts | spawn 选项 | 生命周期流 |
| 2.3 | 不内部决定 spawn 目标 | pi-rpc-process.ts | `SpawnSpec` 输入(自 @blksails/pi-web-protocol) | — |
...
| 5.3 | 输入输出用 protocol 类型 | pi-rpc-process.ts | `@blksails/pi-web-protocol` | — |
...
|-----------|-------|--------|--------------|--------------------------|-----------|
| `PiRpcChannel`(pi-rpc-channel.ts) | port | 传输无关通道接口 + Health 类型(SpawnSpec 自 protocol 导入) | 1.1–1.5, 6.4 | @blksails/pi-web-protocol (P1) | Service |
| `JsonlLineReader`(jsonl-line-reader.ts) | framing | 增量 JSONL 成帧 | 3.1–3.6, 1.2, 4.6, 7.1 | — | Service |
| `PiRpcProcess`(pi-rpc-process.ts) | local adapter | spawn + 三类分发 + 命令封装 + 生命周期 | 2.x, 4.x, 5.x, 6.x | PiRpcChannel (P0), JsonlLineReader (P0), @blksails/pi-web-protocol (P0), child_process (P0) | Service, Event, State |
...
- 同文件定义 `ChannelHealth`(健康状态形状,Req 6.4);`SpawnSpec`(`{ cmd, args, cwd, env }`)不在此定义,而是由 protocol-contract 拥有并导出,本特性经 `import type { SpawnSpec } from "@blksails/pi-web-protocol"` 消费(单一事实来源,见 Req 2.1)。
- 仅为类型/接口声明,无运行时行为,使其可被 mock 替换(Req 1.5)。

**Dependencies**
- External: `@blksails/pi-web-protocol` — 若 health 形状引用协议常量则消费,否则纯本地类型 (P1)
...
import type { SpawnSpec, /* RpcExtensionUIResponse 等 */ } from "@blksails/pi-web-protocol";
...
- 命令封装:18 个方法各生成唯一 `id`、构造命令帧经 `send` 写出、返回待决 Promise(Req 5.1/5.2);输入输出类型取自 `@blksails/pi-web-protocol`(Req 5.3);异步不阻塞(Req 5.4)。`onEvent()` 订阅事件,`respondExtensionUI(id, …)` 写回 stdin 并清待决(Req 5.5/4.4)。
...
- External: `@blksails/pi-web-protocol` — 命令/响应/事件/扩展 UI 类型,以及 `SpawnSpec` 类型(protocol-contract 拥有并导出) (P0)
...
```
> 各命令方法的精确入参与返回子类型取自 `@blksails/pi-web-protocol` 的 `RpcCommand`/`RpcResponse` 判别联合;此处以 `RpcResponse` 占位表示按 `id` 关联后兑现的具体响应。命令帧的 `id` 生成与关联是本特性职责,命令负载形状是 protocol 职责。
...
- **序列化格式**:JSON over JSONL(每行一个 JSON 对象);本特性负责成帧与 `id` 关联,负载形状由 `@blksails/pi-web-protocol` 定义。
...
- **类型来源**:命令/响应/事件/扩展 UI 类型,以及 `SpawnSpec`,一律 import 自 `@blksails/pi-web-protocol`,本特性不重定义(单一事实来源);`SpawnSpec` 由 protocol-contract 拥有并导出。
