...
通道对外是同一套 RPC JSONL 协议,因此无论上游给出的 spawnSpec 指向 bootstrap runner(`runRpcMode`)还是 `pi --mode rpc`(`node <pkg>/dist/cli.js --mode rpc`),通道实现完全复用。所有跨层协议类型(`RpcCommand` / `RpcResponse` / `AgentEvent` / `RpcExtensionUIRequest` / `RpcExtensionUIResponse` 等)以及 `SpawnSpec`(`{ cmd, args, cwd, env }`)由上游 `protocol-contract`(`@blksails/protocol`)拥有并导出,本特性只经 `import type … from "@blksails/protocol"` 消费,不重定义(单一事实来源)。
