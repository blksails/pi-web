...
- **Key Findings**:
  ...
  - 协议类型已由上游 `protocol-contract`(`@blksails/pi-web-protocol`)集中导出(`RpcCommand`/`RpcResponse`/`AgentEvent`/`RpcExtensionUIRequest`/`RpcExtensionUIResponse` 等),本特性只 import 消费,不在本地重建 `rpc-types.ts`(PLAN.md 早期方案已被 protocol-contract 取代)。
...
- **Selected Approach**:从 `@blksails/pi-web-protocol` import `RpcCommand`/`RpcResponse`/`AgentEvent`/`RpcExtensionUIRequest`/`RpcExtensionUIResponse` 等。
