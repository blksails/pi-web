...
## 1. Codec 注入接缝
- [ ] 1.1 `ConfigCodec` 接受注入 `WorkspaceNamespace`
...
- [ ] 1.2 `SourceSettingsCodec` 接受注入 `Workspace`（双根）
...
## 2. 路由工厂 + 装配透传
- [ ] 2.1 `createConfigRoutes` / `createSourceSettingsRoutes` 加 `workspace?` opt
...
- [ ] 2.2 `HostDeps` 加 `workspace?`/`adminPolicy?` + defaultCapabilities 透传
