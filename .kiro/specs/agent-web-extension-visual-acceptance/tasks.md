...
> - ✅ **更正**:baseline **可构建可运行**(`next dev` 在 3000 返回 200,扩展管线正常)。此前"baseline 不可构建"系误读 LSP 噪声(陈旧 dist/project-reference + `ignoreBuildErrors:true` 下不阻断);`zod` 实为 pnpm 嵌套已装,`@blksails/pi-web-agent-kit` 不在 web app 构建路径。
