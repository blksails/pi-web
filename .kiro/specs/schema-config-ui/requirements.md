...
### R6 分层与同构契约
- 6.1 THE 表单 IR 类型与各配置域 schema SHALL 定义在 `@pi-web/protocol`(零运行时依赖,除 zod),供前后端共用。
- 6.2 THE 渲染层 SHALL 位于 `@pi-web/ui`,状态/校验 hook 位于 `@pi-web/react`,持久化端点位于 server/app,遵循 protocol→react→ui→app 单向依赖。
