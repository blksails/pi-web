...
```ts
// @blksails/protocol/src/config/form-schema.ts(类型;运行期可选 zod 校验)
...
```ts
// @blksails/react  src/config/settings-registry.ts
...
import { defaultSettingsRegistry } from "@blksails/react";
import { authFormSchema, settingsFormSchema } from "@blksails/protocol/config";
import { makeConfigDomainIO } from "@blksails/react"; // 基于 /api/config/:domain
...
```
@blksails/ui  src/config/settings-shell.tsx
