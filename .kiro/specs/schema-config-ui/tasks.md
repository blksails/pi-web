...
- [x] 1.1 定义表单 IR 类型与 UI 元数据解析
  ...
  - 完成态:protocol 导出上述类型与 `parseDescribeMeta`;`pnpm --filter @blksails/pi-web-protocol typecheck` 通过;元数据解析正反例单测通过
  - _Requirements: 1.3, 6.1_
  - _Boundary: @blksails/pi-web-protocol config 子面_
...
- [x] 3. 核心:状态与校验(@blksails/pi-web-react)
...
- [x] 5. 集成:前端设置系统与注册机制
- [x] 5.1 设置面板注册表(@blksails/pi-web-react)
...
- [x] 5.2 `useConfigDomain` 与 `makeConfigDomainIO`(@blksails/pi-web-react)
...
- [x] 5.3 设置外壳 `<SettingsShell>`(@blksails/pi-web-ui)
