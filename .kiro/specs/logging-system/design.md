...
**Impact**: 新增一个零依赖同构日志库 `@pi-web/logger`，并在既有传输（SSE control 帧）、配置（静态 schema 框架）、面板（PiSessionStats 范式）三套既有机制上做薄接入，不引入新的传输/配置框架。Node 端日志走子进程 **stderr**（带 sentinel 标记），主进程解析后经 `control:"logs"` 帧实时推送、并入环形缓冲供 REST 拉取。
...
### This Spec Owns
- 零依赖同构日志库 `@pi-web/logger`：`createLogger` / `Logger` / `child` / 级别与命名空间门控 / Node 与浏览器双 sink / 浏览器侧 ring buffer 总线。
...
- `@pi-web/logger`：零运行时依赖（仅 TS 类型 + 标准 JS）。不得 import protocol/react/server/任何 Node 专属模块到浏览器路径。
- `protocol` 可 **type-only** 依赖 `@pi-web/logger` 的 `LogEntry` 形状以对齐 wire schema（logger 零依赖，无环）。
...
|-------|------------------|-----------------|-------|
| 日志库 | 新建 `@pi-web/logger`（TS strict，零运行时依赖） | 同构 createLogger / sink / 浏览器总线 | 不得引入 Node 专属 import 到浏览器路径 |
...
**依赖方向（左→右，禁反向 import）**：
`@pi-web/logger` → `protocol` → `agent-kit` / `web-kit` → `server` / `react` → `ui` → `app`（含 `lib/settings`）。
...
```
packages/logger/
├── package.json            # name @pi-web/logger，零运行时依赖，exports 双入口
├── tsconfig.json
└── src/
...
|-----------|-------|--------|-----|----------|-----------|
| `@pi-web/logger` | lib | 同构日志产出 + 浏览器总线 | 1, 2.3 | — | Service, State |
