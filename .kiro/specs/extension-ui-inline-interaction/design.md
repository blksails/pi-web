...
### Non-Goals
- 不改 `@pi-web/protocol` 请求/响应结构与 `useExtensionUI` hook。
...
- `@pi-web/react`：`UseExtensionUIResult`（`queue`/`current`/`respond`/`error`/`pending`）。
- `@pi-web/protocol`：`RpcExtensionUIRequest`、`UiResponseRequest`（仅类型消费）。
- `@pi-web/ui` 内部：`ui/card.tsx`、`ui/button.tsx`、`lib/cn.ts`、`lucide-react` 图标。
...
- `@pi-web/ui` 公开入口移除/重命名 `PiInteraction` 导出。
...
|-------|------------------|-----------------|-------|
| Frontend | React 18/19 + `@pi-web/ui`（shadcn Card/Button + Tailwind v4） | 内联交互卡渲染 | 复用既有等价层，不引入 AI Elements 包 |
| Frontend | `lucide-react` | 终态/请求图标 | 既有依赖 |
| 数据 | `@pi-web/react` `useExtensionUI` | 请求队列与回传（不改） | 仅消费 |
| 类型 | `@pi-web/protocol` | 请求/响应类型（不改） | 仅类型导入 |
