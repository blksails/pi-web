...
- **agent-runner**:`runner.ts`(`startRunner`→`createAgentSessionRuntime`→`runRpcMode`)、`option-mapper`(`customTools` 注入链路)、`agent-loader`(jiti + `@blksails/agent-kit` 别名)、`assemble-spawn`(`buildEnv` spawn env 下发)、`AgentContext`。
...
|-------|------------------|-----------------|-------|
| Frontend / CLI | `@blksails/ui`(展示既有 `/raw` URL) | 产出物经分发 URL 展示 | 复用 attachment-store 展示链路,不新增前端逻辑 |
| Backend / Services | `@blksails/server`(新增 `attachment-bridge/` 模块 + runner hook 接线 + prompt 注入接缝) | L2 resolve + 子进程 store + 闸门 + 回流 + 注入 | 复用 `attachment-store` 的 store 工厂 |
| Agent SDK | `@blksails/agent-kit`(暴露 store 给 `defineTool.execute`)+ 示例 tool | tool 接入范式 | `defineTool` re-export pi |
...
└── attachment.ts               # 重导出 AttachmentToolContext / resolve 句柄类型，供 tool 作者按 @blksails/agent-kit 引用
...
- `packages/server/src/index.ts` / `packages/agent-kit/src/index.ts` — barrel 导出 attachment-bridge 公共类型与 tool context、`@blksails/agent-kit` 暴露 tool 作者用句柄类型。
...
import type { Attachment } from "@blksails/protocol";          // 上游描述符 DTO（不含字节）
import type { BlobMeta } from "@blksails/server";              // 上游导出的 meta 类型（{ mimeType, size }）
...
import type {
  ...
  PutInput,
} from "@blksails/server"; // attachment-store 受认可复用面（barrel 导出）
...
```
- 接入方式:runner 装配 `customTools` 时,把 `AttachmentToolContext`(闭包绑定子进程 store + 当前 sessionId)经 tool 工厂注入;tool 作者经 `@blksails/agent-kit` 引用类型(4.1)。
