...
> Wave 1:text_to_image + image_edit,纯执行→落库→默认卡片闭环。引擎承载于新建 `@pi-web/tool-kit`(双入口)。
> spike-first:**任务 4** 是首个端到端可验证点(text_to_image + 单 provider 跑通),据此确认「移植精简引擎」策略;任务 5 起再扩展。

- [ ] 1. Foundation:`@pi-web/tool-kit` 包脚手架与引擎类型
- [x] 1.1 创建 `@pi-web/tool-kit` 包骨架(双入口)并接入 workspace
  ...
  - 在仓库根 `pnpm install` 后,其它包可解析 `@pi-web/tool-kit` 与 `@pi-web/tool-kit/runtime`;`pnpm -F @pi-web/tool-kit build` 成功
