...
引擎承载于新建独立包 `@blksails/pi-web-tool-kit`(通用工具套件,AIGC 为首批工具集),移植 pi-labs aigc 引擎的精简版:保留零依赖的声明式 `Category/Variant/EndpointBehavior` 与执行内核,去除其 DB/S3 耦合,把"产物稳定化"接缝换成 pi-web 附件落库。
...
- 建立可声明式扩展的 `@blksails/pi-web-tool-kit`,AIGC 工具集与未来工具集并列、互不影响内核。
...
### This Spec Owns
- 新建包 `@blksails/pi-web-tool-kit`:声明式工具引擎(类型 + 执行内核 + env 变量解析)与 AIGC 首批工具集(`text_to_image`/`image_edit` 的 category 声明 + provider 变体)。
...
- `@blksails/pi-web-agent-kit`(**type-only**:`AttachmentToolContext`/`AttachmentToolHandle`/`ToolOutputRef`、`defineAgent`)。
- `@blksails/pi-web-protocol`(经 agent-kit 间接,`Attachment` 类型)。
...
- **禁止**:`@blksails/pi-web-tool-kit` → `@blksails/pi-web-server` 的值依赖(仅约定常量同值);主入口禁止顶层 import pi SDK/undici。
...
- 引入前端对 `@blksails/pi-web-tool-kit` 主入口的 import(触发元数据下发设计,后续 Wave)。
...
|-------|------------------|-----------------|-------|
| Backend / Services | `@blksails/pi-web-tool-kit`(新包,TS strict) | 声明式引擎 + AIGC 工具集 | 双入口;主入口零运行时依赖 |
...
### Directory Structure
```
packages/tool-kit/                       # 新建 @blksails/pi-web-tool-kit
├── package.json                         # exports: "." 主入口 + "./runtime" 子入口; peerDeps: pi SDK/pi-ai; deps: undici, @blksails/pi-web-agent-kit
