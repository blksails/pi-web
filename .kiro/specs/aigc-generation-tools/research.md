...
- 作者面契约 `AttachmentToolContext`(`@pi-web/agent-kit`):`available` / `resolve(id)→AttachmentToolHandle` / `putOutput({bytes,name,mimeType})→ToolOutputRef{attachmentId,displayUrl,name,mimeType}`。
...
### D-2 包与入口分层(守打包边界)
`@pi-web/tool-kit` 双入口:
- 主入口 `@pi-web/tool-kit`:引擎**类型** + AIGC **category 声明**(纯数据/纯函数,**禁止顶层 import** pi SDK / undici);前端安全、可序列化元数据归此。
- 子入口 `@pi-web/tool-kit/runtime`:`compileCategory`(`defineTool` 包装)+ `runEndpoint`(undici 代理)+ env var-resolver + 落库适配;**node-only**,由 agent index.ts / runner 装配导入。
...
### D-5 seam key 常量去重
`@pi-web/tool-kit` 不依赖 `@pi-web/server`;seam key `"__piWebAttachmentToolContext__"` 在 tool-kit 内自声明同值常量(与 server / 示例工具同一约定),仅 **type-only** 依赖 `@pi-web/agent-kit` 的 `AttachmentToolContext`。
