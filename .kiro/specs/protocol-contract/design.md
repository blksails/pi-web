...
**Purpose**:本特性交付 `@pi-web/protocol`——pi-web 全项目的唯一契约根。它是一个零运行时依赖(除校验库)、同构(Node + 浏览器)、纯 TypeScript 的协议包,集中定义并以 zod schema 在运行时校验所有跨层契约。
...
```
packages/protocol/
├── package.json                  # name @pi-web/protocol, exports, deps 仅 zod;sideEffects false
