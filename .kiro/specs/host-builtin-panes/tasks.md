...
- [x] 6.2 (P) 注入等价性与分派测试
  ...
  > **2026-07-30 改向(用户决策:废弃 `slots.panelRight`,全量重写为 pane)。**
  > 旧槽路径将在下游 spec 里**整条删除**,故本任务原定的「两条路径注入**等价**」与
  > 「旧槽走旧路径」断言写了即要删 —— 且后者 6.1 的「旧槽形态不回退」describe 块已覆盖
  > (随删除一并移除)。本任务因此收窄为**三条在删除后依然成立**的断言:
  >   ① 宿主路径注入面的**完整性**(以旧槽路径实收 prop 键集合为事实源派生,见下);
  >   ② 拒绝清单的**上报时机**;
  >   ③ 镜像↔canonical 双向可赋值。
  > 另**新增**一条过渡期断言:声明旧槽即产生**废弃诊断**(原实现只在旧槽 ∧ 声明键并存时才诊断,
  > 那覆盖不到「只声明旧槽」的绝大多数存量 agent —— 而它们正是待迁移的对象)。

  - ★ 注入面完整性以**旧槽路径实收的 prop 键全集**为事实源派生断言,而非测试里手写清单 ——
    手写清单的话,新增一个注入项只接到旧槽路径、忘了宿主路径,测试照样全绿
  - 断言声明旧槽的 agent 产生**废弃诊断**,且诊断指明迁移途径(pane 声明键)
