...
**Users**: pi-web 终端用户(更现代的多模态对话交互)与集成方(可复用的 `@blksails/pi-web-ui` 富组件)。

**Impact**: 在 `@blksails/pi-web-ui` 新增元件层与装配层;在 `@blksails/pi-web-react` 新增数据 hooks 与 transport 附件映射;在 `@blksails/pi-web-protocol`/`@blksails/pi-web-server` 补齐三个**已存在 RpcCommand**(`get_available_models`/`fork`/`get_fork_messages`)的 REST 薄透传;app-shell 切到 `<PiChatPro>`。
...
### This Spec Owns
- **UI 元件层**(`@blksails/pi-web-ui/src/elements/*`):无状态 AI Elements 等价组件(conversation/message+branch/prompt-input/attachments/model-selector/speech-input/web-search-toggle/sources/suggestions/submit-button)。
  - 其中 **attachments 呈现增强**(Req 12):图片缩略图的悬停大图预览、布局变体(inline/grid/list)、附件类型标签与无缩略图时的占位图标降级;真实处理边界不变(仍仅图片入列,承接 Req 3)。
- **UI 装配层**(`@blksails/pi-web-ui/src/chat/pi-chat-pro.tsx`):`<PiChatPro>` 组合元件 + hooks + 渲染器注册表。
- **数据 hooks**(`@blksails/pi-web-react/src/hooks/*`):`useModels`/`useAttachments`/`useBranches`/`useSuggestions`。
...
- 依赖方向:`@blksails/pi-web-protocol` → `@blksails/pi-web-server`;`@blksails/pi-web-protocol` → `@blksails/pi-web-react` → `@blksails/pi-web-ui` → app。各层只向左依赖,不得反向。
...
- `PendingAttachment` 形状(`@blksails/pi-web-react`)新增/变更字段(如携带 mime/size)→ attachments 类型标签与占位图标推导(Req 12)重校。
...
| Frontend (data) | `@ai-sdk/react` useChat + `@blksails/pi-web-react` hooks | 流式态、附件、模型、分支、建议接线 | 复用既有 transport/controls |
| Protocol | `@blksails/pi-web-protocol`(zod,零运行时依赖) | 新增三 REST DTO | 复用既有 `Model`/`ImageContent` schema |
| Backend (server) | `@blksails/pi-web-server`(Node runtime) | 三透传方法 + 三路由 | 镜像 setModel/commands |
