...
- **Discovery Scope**: Extension(扩展 `@blksails/ui` + `@blksails/react`,并薄透传扩展 `@blksails/protocol` + `@blksails/server`)
- **Key Findings**:
  - `@blksails/ui` 已有 `<PiChat>`、`PartRenderer`、`RendererRegistry`(单例+工厂)、`PiChatSlots`(header/footer/sidebar/messageActions)、`cn()`、自建 button/card/dialog/select 与 `streamdown` 的 `<Response>`、`PiReasoning`、`PiToolPart`。依赖已含 radix-dialog/radix-select、cva、clsx、lucide、streamdown、tailwind-merge —— **无需新增 npm 依赖**即可实现 AI Elements 等价元件。
  - `@blksails/react` 已有 `usePiSession`/`usePiControls`/`useExtensionUI`、`PiTransport`、`createPiClient`、`ControlStore`(SSE 旁路 stats/extensionUiQueue)。`usePiControls` 已暴露 `getCommands()`+`commands` 状态。`PiClient` 已有 `getCommands/getMessages/getStats/getState` 与 `setModel/abort/...` 写方法,但**缺** `getAvailableModels/fork/getForkMessages`。
  - `@blksails/protocol` 的 `RpcCommand` 已定义 `get_available_models`/`fork`/`get_fork_messages`/`get_commands`;`PromptRequest` 已支持 `images?: ImageContent[]`(`{type:"image",data,mimeType}`)。但缺这三者的 **REST DTO**。
  - `@blksails/server` HTTP 路由为**逐命令显式注册**(`create-handler.ts` 注册表 + `routes/{command,query}-routes.ts`,每能力一个 `PiSession` 方法)。`set_model`→`POST /sessions/:id/model`、`commands`→`GET /sessions/:id/commands` 为既有范本。**无通用 control 透传端点**。
...
|--------|-------------|-----------|---------------------|-------|
| A. 富组件进 `@blksails/ui` + 薄透传补齐后端(选用) | 元件层 + `<PiChatPro>` 装配层 + react 数据 hooks + protocol/server REST 薄透传 | 可复用、端到端可用、符合 §13.1 分层开放包 | 触及 4 个包,但均为低风险镜像式扩展 | brief 决策 A |
...
- **Trade-offs**: 触及 `@blksails/server`/`PiSession`(超出 brief 原列的 ui/react),但仅加镜像式方法,不改会话语义。
- **Follow-up**: 实现时确认 `PiSession` 底层 RpcChannel 能直接发送这三 command;e2e 验证模型切换与分支切换。

### Decision: 新增 `<PiChatPro>` 与现有 `<PiChat>` 并存
- **Selected Approach**: `@blksails/ui` 新增导出 `<PiChatPro>`;`<PiChat>` 保持不变;app-shell 切到 `<PiChatPro>`。
...
- **Rationale**: AI Elements 走 shadcn registry「拷源码」分发,直接照搬会带入 `@/` 别名假设、额外 Radix 依赖与第二套主题 token,破坏 `@blksails/ui` 作为**可发布 npm 包 + 零硬编码主题 + 不新增运行时依赖**的既定约束(见 `## Design Decisions › AI Elements 等价元件自实现`)。故借其 API/交互模式,自实现适配。
...
- AI SDK v5 `useChat` / `ChatTransport` —— 流式态与消息发送契约(已在 `@blksails/react` 实现)
