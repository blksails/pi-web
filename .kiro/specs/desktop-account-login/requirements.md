...
### Requirement 13:「暂不登录」出口

> 2026-07-28 由打包态烟雾测试撞出。Req 10(门禁)与 Req 11(随包固化云端地址)叠加后,
> **任何新机器上云端都算「已配置」**,未登录即被拦死 —— 没有云端账号的人**永远进不去**。
> CI 机器等聊天输入框等到超时,而它的处境与一个普通新用户完全相同。

**Objective:** As a 没有云端账号的 pi-web 桌面用户, I want 不登录也能用本地功能, so that 云端账号不是使用这个工具的前提。

#### Acceptance Criteria

1. The 登录页 shall 提供「暂不登录,仅使用本地功能」的出口。
2. When 用户选择暂不登录, the pi-web 宿主 shall 立即进入主界面,本地 agent 与本地模型照常可用。
3. The 该选择 shall 被记住 —— 下次启动不再拦截(否则等于没给出口)。
4. While 处于「暂不登录」态, the 登录入口 shall 仍在头部可见,用户可随时登录。
5. When 用户此后登录成功, the pi-web 宿主 shall 撤销「暂不登录」记忆(用户已表达相反意愿)。
6. If 记忆用的存储不可用(隐私模式等), then the pi-web 宿主 shall 不崩溃,该选择在本次会话内仍然生效。
