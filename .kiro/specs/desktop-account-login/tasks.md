...
3. **重启后自动登录未真机验证**。读的那半边(keychain → base_env → 播种 → `current()` 补加载)
   ...
   它需要真实账号登录一次,只能由持有账号的人完成。

### 任务 13 真机验证(打包态,2026-07-27)

端点挂载与门控这两条我能自己验,已验:

| 场景 | 结果 |
|---|---|
| 打包态 + 不带 token | **401** —— 路由已挂载,证明 `PI_WEB_SHELL_TOKEN` 确实到达了 server |
| 打包态 + 错误 token | **401** |
| **非桌面**(`pnpm dev`,无 shell token) | **404** —— 路由**压根不存在**,不是「存在但拒绝」(Req 12.7) |

401 与 404 的区别正是这里要证的:打包态是 401(有路由、门在),dev 态是 404(连路由都没有)。
...
## 15. Req 13 · 「暂不登录」出口(2026-07-28)

- [x] 15.1 门禁增加出口
  - `use-identity` 加 `skipped` / `skipLogin()`,记忆落 localStorage(`pi-web:skip-login`);
    登录成功即撤销该记忆
  - `LoginPage` 加「暂不登录,仅使用本地功能」按钮 + 一句说明
    (本地 agent/模型无需登录;登录后额外获得线上源与云端模型)
  - `IdentityGate` 在 `skipped` 时放行
  - 存储不可用(隐私模式)时不崩,降级为「本次会话内有效」
  - _Requirements: 13.1, 13.2, 13.3, 13.4, 13.5, 13.6_

- [x] 15.2 测试(3 例)
  - 点击即进主页面 / **选择被记住**(重新挂载不再拦)/ localStorage 抛错时不崩
  - 第二条是重点:不记住等于没给出口,用户每次开应用都要再点一次
  - _Requirements: 13.2, 13.3, 13.6_

- [x] 15.3 打包态烟雾测试走该出口
  - `e2e/desktop/shared.mjs` 在等聊天输入框前,先点「暂不登录」(可见才点)
  - ★ **不用 env 绕过** —— 那样测的就不是用户实际会走的路径。点真按钮,
    顺带把「出口确实能用」纳入打包态验收:它没了,本地用户就被锁死
  - _Requirements: 13.1, 13.2_

### 这个缺陷的来历(值得记)

Req 10 与 Req 11 单独看都对,**叠加后产生了谁都没要求的行为**:桌面版从
「可以登录」变成了「必须登录」。我在实现时没意识到这一点,是打包态烟雾测试
在一台无账号的机器上撞出来的 —— 那台机器的处境和一个普通新用户一模一样。

★ 单测覆盖不到它:两个 Req 的测试各自都绿,因为它们从不共同出现。
