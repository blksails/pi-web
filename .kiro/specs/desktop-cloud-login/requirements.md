...
### Requirement 8: 云端地址的可配置性（实施后补充：打包态无法启用登录）

**Objective:** 作为桌面用户，我想在应用内配置云端地址并使登录入口出现，以便不必从命令行带环境变量启动就能使用云端登录。

**背景（实测缺口）**：Req 1–7 已交付登录组件、鉴权端点、凭据解析与钥匙串存取，但**用户没有任何途径告诉应用「云端在哪」**。云端地址此前只能来自进程环境变量：桌面壳的 `base_env()` 不转发它；Finder 双击启动的应用没有 shell 环境；服务端的 `.env` 从 cwd 读，而打包态 cwd 是每次升级即更换、且会被 GC 清理的运行时目录。实测结果：双击打开的桌面版 `GET /api/auth/me` 返回 404（鉴权能力面未注册），登录入口不渲染。故 Req 1.1「用户可发起登录」在打包态实际不成立。

#### Acceptance Criteria

1. The 系统 shall 提供一个持久化的用户级配置项用于指定云端出口地址，其存放位置不随应用升级而失效。
2. When 用户在设置界面填入云端出口地址并保存, the 系统 shall 将其持久化，且在下次启动后仍然生效。
3. While 已配置合法云端出口地址, when 应用启动, the 系统 shall 启用登录能力（鉴权端点可用、登录入口可见），无需任何环境变量。
4. Where 进程环境已提供云端出口地址, the 系统 shall 以环境变量优先，使既有的命令行与自动化用法不被破坏。
5. While 既未配置也未提供环境变量, the 系统 shall 保持未启用登录的现状行为（无登录入口、无鉴权端点），不因此报错。
6. If 配置的地址非法, then the 系统 shall 在保存时拒绝并说明原因，不写入非法值。
7. The 系统 shall 在设置界面明示「修改后需重启应用才生效」，避免用户误以为改动即时生效。
8. The 配置项 shall 对浏览器部署与命令行部署同样有效，不是桌面壳专属机制。
