...
**Impact**:在 pi-web 自有传输契约层(`transport/*`)新增一个 `data-pi-ui` part,复用既有 SSE/transport/`useChat`/`PartRenderer`/`RendererRegistry` 管线;在 `@blksails/pi-web-ui` 新增内置组件库 + 组件注册表 + 沙箱解释器 + part 渲染器。**不新增传输机制,不改 pi 原生 `extension-ui.ts`。**
...
完成判据:worktree 内 `pnpm --filter @blksails/pi-web-protocol test`、`pnpm --filter @blksails/pi-web-ui test`、两包 `typecheck` 均以新鲜运行输出通过。
