...
3. While 调用未指定模型 and 已配置默认视觉模型 and 该模型在候选清单中，the 视觉识别工具 shall 使用该默认模型且不提示选择——**无论交互式界面是否可用**。
...
> **4.3 为何不以「交互式界面不可用」为前提**（2026-07-17 修订）
>
> `hasUI` 表示「会话具备 UI 能力」，不表示「此刻有人在看」。经 IM 通道（pi-gateway 企微）
> 或定时任务驱动的会话 `hasUI` 同为 true，弹层没有任何人会点 → `await ui.select` 永不
> resolve → 工具静默挂死（无异常、无结果、无超时），恰好违背本 Requirement 的 Objective。
> 运营方显式配置默认模型即表达「别问，用这个」，故默认模型优先于交互，与 aigc
> `IMAGE_GENERATION_DEFAULT_MODEL` 的语义对齐。未配置默认时，有 UI 仍照常弹层（Req 3.1）。
