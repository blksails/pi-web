...
- [x] 1.1 创建 `@pi-web/ui` 包骨架与构建/测试配置
...
- [x] 1.2 引入 AI Elements 与 shadcn 底座组件源
...
- [x] 2. 测试夹具(mock 会话与消息样本)
- [x] 2.1 实现 mock 会话与 mock transport 夹具
...
- [x] 3. 渲染器注册表(扩展点)
- [x] 3.1 实现 `renderer-registry.ts`(注册/解析/默认回退/覆盖) (P)
...
- [x] 4. parts 层默认渲染组件
- [x] 4.1 实现 `<PiToolPart>` 工具卡(start/update/end 三态) (P)
...
- [x] 5. part 分派器
- [x] 5.1 实现 `PartRenderer` 分派(含注册表解析与默认回退)
...
- [x] 6. controls 层组件
- [x] 6.1 实现 `<PiModelSelector>` 与 `<PiThinkingLevel>` (P)
...
- [x] 6.2 实现 `<PiSessionStats>` (P)
  ...
  - _Depends: 1.2, 2.1_
- [x] 6.3 实现 `<PiCommandPalette>` 斜杠命令补全
...
- [x] 7. dialog 层权限弹窗
- [x] 7.1 实现 `<PiPermissionDialog>`(扩展 UI 四类 + 回传)
...
- [x] 8. `<PiChat>` 装配组件与插槽
- [x] 8.1 实现 `<PiChat>` 装配(useChat + AI Elements + part 分派 + 内嵌弹窗/控制)
  ...
  - _Depends: 5.1, 7.1_
- [x] 8.2 实现 `<PiChat>` 四个插槽(header/footer/sidebar/messageActions)
...
- [x] 9. 分发产物(npm 导出 + shadcn registry)
- [x] 9.1 实现 npm 聚合导出面 `index.ts`
...
- [x] 9.2 生成 shadcn registry 清单(`npx pi-web add chat`)
...
- [x] 10. Storybook 与 e2e(硬性)
- [x] 10.1 编写 Storybook 故事(可视化文档 + e2e 场景)
  ...
  - _Requirements: 11.6_
  - _Depends: 8.2, 2.1_
- [x] 10.2 实现 e2e:mock 会话驱动 `<PiChat>` 全交互
