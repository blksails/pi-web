...
**Purpose**:本特性交付 `@blksails/pi-web-ui`——pi-web 的**有样式、可主题化、可扩展**的浏览器组件层。它把 Vercel AI Elements(`Conversation/Message/Response/Reasoning/Tool/PromptInput/Actions`)与 `@blksails/pi-web-react` 的 headless 层(`PiTransport`/`usePiSession`/`usePiControls`/`useExtensionUI`/`createPiClient`)装配为一个可拖入的 `<PiChat>`,并提供细粒度组件、渲染器注册表与布局插槽。

**Users**:想快速拥有成品聊天 UI 的第三方 React/Next 集成方(§13.3 方式 A),以及本项目整站 `app-shell`(消费本层闭合全链路 e2e)。

**Impact**:把 `PLAN.md` §1(AI Elements 装配)、§4(pi 事件→UIMessage 渲染映射)、§13.1(`@blksails/pi-web-ui` 导出面)、§13.4③(渲染器注册表/插槽)收敛为一个边界清晰、可单测、可主题化的有样式前端层。本 spec **消费**上游契约(`@blksails/pi-web-react` 的 transport/hooks、AI SDK v5 的 `useChat`/`UIMessage` part 结构、AI Elements 组件 API、shadcn/ui CSS 变量),不重定义、不触达后端。
...
- 分发产物:`@blksails/pi-web-ui` 的 npm 聚合导出面;shadcn registry 清单(`registry.json` + 组件源条目);CSS 变量主题层。
...
- 服务端真值会话状态——本层只持视图态,消息/控制/扩展 UI 数据来自 `@blksails/pi-web-react` 派生状态。

### Allowed Dependencies

- **上游 spec(运行时)**:`@blksails/pi-web-react`(`usePiSession`/`usePiControls`/`useExtensionUI`/`PiTransport`/`createPiClient`/`PiProvider` 及其类型)。经它间接获得 `@blksails/pi-web-protocol` 的类型(如 `ExtensionUIRequest`/`StatsResponse`/`CommandsResponse`/data-part type),本层从 `@blksails/pi-web-react` re-export 的类型消费,不直接 `import @blksails/pi-web-protocol` 形状定义。
...
- **依赖方向**:`react-client ← ui-components`;`ui-components ← app-shell`(下游)。禁止反向。本层**不**依赖 `@blksails/pi-web-server`/`session-engine`/`http-api` 的任何运行时对象。
...
- `@blksails/pi-web-react` 的 hooks 签名(`usePiSession`/`usePiControls`/`useExtensionUI`)、`PiTransport` 构造或 re-export 类型(`ExtensionUIRequest`/`StatsResponse`/`CommandsResponse`/data-part type)变更。
...
- **Steering compliance**:TypeScript strict、禁 `any`;浏览器环境;有样式但主题经 shadcn CSS 变量继承宿主(structure.md/§13.4);前后端经协议解耦,本层经 `@blksails/pi-web-react` 消费(roadmap/tech.md);仅依赖允许的上游(brief 约束)。
...
| Backend / Services | — | 本层不含后端;经 `@blksails/pi-web-react` 消费 | 不依赖后端对象 |
| Data / Storage | 无服务端状态;仅前端视图态(折叠/面板/弹窗焦点) | 组件局部 state | 真值在 `@blksails/pi-web-react` 派生态 |
| Messaging / Events | 消费 `useChat` 的 `UIMessage.parts` 流式更新 + hooks 旁路态(扩展 UI/stats/commands) | part 分派渲染 + 控制呈现 | 不触达 SSE/REST |
| Infrastructure / Runtime | AI SDK v5(`ai`/`@ai-sdk/react`);Vercel AI Elements;shadcn/ui(Radix + Tailwind v4);`@blksails/pi-web-react`;`vitest` + `@testing-library/react` + `@testing-library/user-event` + DOM 环境 + Storybook(e2e) | 运行与测试 | shadcn CSS 变量主题 |
...
```
packages/ui/
├── package.json                  # name @blksails/pi-web-ui;deps: 无后端;peerDeps: react, @blksails/pi-web-react, ai, @ai-sdk/react;含 AI Elements/shadcn 生成的源(随 registry 分发);sideEffects 谨慎(注册表副作用经显式 API)
...
- 无(greenfield 新包)。若 monorepo workspace 已存在,需将 `packages/ui` 纳入 workspace 并接入 `@blksails/pi-web-react`、`react`、`ai`、`@ai-sdk/react`、AI Elements、shadcn——接线随仓库初始化处理,本 spec 创建包自身文件与测试。
...
`<PiChat>` 不感知传输细节:它只读 `useChat` 的 `messages[].parts` 并交给 `PartRenderer`;part 的来源(SSE 解码)由 `@blksails/pi-web-react` 负责。
...
队列入/出与回传端点由 `@blksails/pi-web-react` 拥有,本层只呈现首项并调用 `respond`。
...
|-----------|-------|--------|--------------|--------------------------|-----------|
| chat/pi-chat.tsx | chat | 装配 useChat(transport) + AI Elements + 插槽 + 内嵌弹窗/控制 | 1.1-1.7,8.1-8.5 | @blksails/pi-web-react (P0), @ai-sdk/react useChat (P0), AI Elements (P0), part-renderer (P0) | Service, State |
...
- External: `@blksails/pi-web-react`(`usePiSession`/`usePiControls`/`useExtensionUI`)(P0)、`@ai-sdk/react` `useChat`(P0)、AI Elements(P0)
...
**Summary-only(有样式,接 `usePiControls`)**:四组件均经 `@blksails/pi-web-react` 的 `usePiControls` 操作/订阅,不向 `useChat` 消息流写入(Req 4.5)。
...
// ExtensionUIRequest 类型经 @blksails/pi-web-react re-export(源自 @blksails/pi-web-protocol)
...
- Postconditions:作答提交后经 `respond` 回传;成功由 `@blksails/pi-web-react` 出队后 `current` 变更关闭弹窗;失败保留。
...
- `index.ts`:npm 聚合导出 `<PiChat>`、各细粒度组件、`registerToolRenderer`/`registerDataPartRenderer`、`createRendererRegistry` 与公开类型(`PiChatProps`/`PiChatSlots`/各 Props/`ToolRenderer`/`DataPartRenderer`)(Req 9.1)。仅依赖 `@blksails/pi-web-react` + shadcn/AI Elements(Req 9.4,经 peerDeps 约束)。

## Data Models

### Data Contracts & Integration

- **核心消费契约**:AI SDK `UIMessage` 与其 `parts`(text/reasoning/tool/`data-pi-*`)——由 `useChat` 提供,本层只读不重定义。扩展 UI 请求类型(`ExtensionUIRequest`)、统计(`StatsResponse`)、命令(`CommandsResponse`)经 `@blksails/pi-web-react` re-export(源自 `@blksails/pi-web-protocol`),本层从 `@blksails/pi-web-react` 消费。
...
- **扩展 UI 回传失败**(Req 6.6):`<PiPermissionDialog>` 保留弹窗 + 显示错误 + 允许重试(出/入队由 `@blksails/pi-web-react` 拥有)。
...
- 本层为浏览器展示层,不持久化凭据、不实现鉴权;所有数据经 `@blksails/pi-web-react` 的 hooks/transport 间接获得,本层不直接发起网络请求。
