...
Greenfield React 组件库(`@pi-web/ui`)。Discovery 类型:**Full(新特性)**,但因上游契约(`@pi-web/react`、`@pi-web/protocol`)与权威设计(`PLAN.md`)已稳定,研究聚焦"如何在既定契约上装配 AI Elements + 落地扩展点",而非外部技术选型探索。
...
- 结论:`@pi-web/ui` 通过 props 接收这些 hooks 结果(或经 `PiProvider`),不重定义其行为;消息流来自 `useChat({ transport })`。
...
- **Boundary**:不绑定后端/路由(`app-shell`),不做非 React 嵌入(`embed-integrations`),仅依赖 `@pi-web/react` + shadcn/AI Elements(brief 约束)。
- **类型来源**:协议派生类型(`ExtensionUIRequest`/`StatsResponse`/`CommandsResponse`/data-part type)从 `@pi-web/react` re-export 消费,避免本层直接耦合 `@pi-web/protocol`,保持单向依赖与单一上游接口。
