...
- `usePiControls`（`@pi-web/react`）→ 提供 `controls.stats`（`SessionStats`）。
...
|-------|------------------|-----------------|-------|
| Frontend | React 19 + shadcn/ui（既有） | 渲染内核用量状态区，复用 `PiSessionStats` | 仅 `@pi-web/ui` 包内 |
| Data | `usePiControls.stats`（`@pi-web/react`，既有） | 提供 `SessionStats` | 不新增数据源 |
