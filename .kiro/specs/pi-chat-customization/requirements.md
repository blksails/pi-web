...
本规格为 pi-web 的 `PiChat` 建立一套面向集成方开发者的「四维可定制契约」,使其在不修改 `@blksails/ui` 源码的前提下完成外观与装配的定制。四个维度为:(1) 主题与 CSS 变量;(2) 整块区域插槽 slots;(3) 细粒度组件覆盖 components;(4) 布局预设 layout 与图标主题 icons。
...
- **In scope**:
  - `@blksails/ui` 包内,单个 `PiChat` 的主题、slots、components、layout、icons 五类公开定制入口(归为四维)。
...
- **Out of scope**:
  ...
  - 协议层 / server 层 / `@blksails/react` / `@blksails/agent-kit` 的改动。
...
3. The `@blksails/ui` 包 shall 导出一份样式预设,使下游构建配置能够以一行引用方式获得本包的令牌到工具类映射,而无需手工重复声明该映射。
