...
**Purpose**: 为 `@pi-web/ui` 的 `PiChat` 提供一套面向集成方开发者的「四维可定制契约」(主题、slots、components、layout/icons),使其在**不修改 `@pi-web/ui` 源码**的前提下完成外观与装配定制。
...
- 协议层 / server 层 / `@pi-web/react` / `@pi-web/agent-kit` 改动 —— 不涉及。
...
- `PiChat` 是无状态装配层:用 `useChat` + `@pi-web/react` hooks 取数据,组合 element 层并以固定 props 接线。定制只需在**装配点**替换实现,不动 hooks。
