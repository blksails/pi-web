...
**Impact**：仅改 app 层 `ChatApp` / `SessionView`。复用既有 `usePiSession` 的「挂载即建会话」语义与 `onSessionId` 的 URL/源映射副作用；不改 `@blksails/ui`、不改 hook、不改协议。
...
- `usePiSession`（`@blksails/react`）现有契约：组件挂载时按 `create` 建会话（`startedRef` 守卫 + `[]` 自动启动）。
