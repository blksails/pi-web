...
- [x] 8. publish：编译、校验与提交

- [x] 8.1 (P) 实现手写清单到发布清单的编译
...
- [x] 10. Wave 2 集成与端到端验证

- [x] 10.1 集成：将 publish 接入分发层
...
## Wave 2 回勾记录(2026-07-24)

Wave 2(publish/registry,任务 7–10)的实现随工作树 `.claude/worktrees/cli-wave2-publish-install`
落地并已在 main 上,但 `tasks.md` 未回勾。本次按 kiro 规范**逐任务用代码证据核实 + 跑测试取
新鲜证据**后回勾,不凭 phase 字段推断。

| 任务 | 证据 |
|---|---|
| 7.1/7.2 注册表端口 + HTTP 适配 | `server/cli/registry/{registry-port,http-registry-adapter}.ts`;`test/registry/http-registry-adapter.test.ts` **11 例**(成功分支 + 错误归一,含「可变 git ref → MUTABLE_REF 且不触网」)。跨仓依赖 `@pi-clouds/registry-client` 经 http-adapter 单点隔离(构建期 inline) |
| 8.1–8.5 publish 编译/签名/校验/投影/编排 | `server/cli/publish/{manifest-compiler,publish-orchestrator}.ts`;`test/publish/publish-orchestrator.test.ts` **34 例**(全链、--dry-run 零外部写、编译失败在任何外部写前终止、DECLARED_PATH_MISSING) |
| 9.1–9.3 安装解析/复核回滚/降级 | `server/cli/install/registry-install.ts`;`test/install/registry-install.test.ts` **11 例**(integrity 复核、字节篡改→回滚 targetDir 不残留、覆盖安装原子替换、git 走直连不经代理) |
| 10.1/10.2 集成 + 契约夹具 e2e | `test/publish/publish-install-e2e.test.ts` **3 例**(发布→安装逐项 integrity 复核、**#28 回归:kind=agent 含 routes 全链**、重复版本 VERSION_REJECTED 无副作用) |
| 10.3* 真实注册表验证 | `test/publish/registry-e2e-online.test.ts` —— `*` 可选,**SKIP**(需真实注册表凭据与网络,离线不跑) |

**新鲜证据**(2026-07-24):`test/publish/ + test/install/ + test/registry/http-registry-adapter`
合计 **66 passed / 2 skipped**(skip 即 10.3* 的 online 用例);核心 publish-orchestrator +
registry-install 单跑 45 passed。

**★ #28 已闭环**:`publish-install-e2e.test.ts:150` 专门覆盖「kind=agent 含 routes 的包能完成
全链发布并被安装」——即 memory 记载的「CLI 发 kind=agent 100% 失败」issue #28 的回归门,已有
测试守住(该 issue 此前已修并发版,此处是 Wave 2 的契约级复现取证)。

至此 cli-package-commands 全 Wave 完成(Wave 1 已随整合线合 main,Wave 2 本次核实回勾)。
