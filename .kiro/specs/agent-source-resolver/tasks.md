...
- [x] 1. 基础:类型契约与测试基础设施
- [x] 1.1 定义共享类型与插件接口(对齐 @blksails/protocol)
  - 在 `lib/pi/source/types.ts` 定义 `ResolvedSource`、`AgentMode`、`TrustDecision`、`TrustFragment`、`ResolveOptions`、`GitSource`、`EntryProbe`、`SourceResolverPlugin`;`SpawnSpec` 必须经 `import type { SpawnSpec } from "@blksails/protocol"` 复用(protocol-contract 为上游拥有者),**不得**在本地定义或重声明。
