...
- **Key Findings**:
  ...
  - spawnSpec 形状须与 `@pi-web/protocol` 对齐并满足 `rpc-channel` local 通道(child_process)直接拉起需求;agentDir 隔离 env 名是 `PI_CODING_AGENT_DIR`(非 `PI_AGENT_DIR`)。
...
- **Findings**:protocol 的 `CreateSessionRequest` 提供入参形状;`SpawnSpec { cmd, args, cwd, env }` 由上游 `@pi-web/protocol`(protocol-contract)**拥有**,本 spec 经 `import type { SpawnSpec } from "@pi-web/protocol"` 复用,不在本地定义或重声明该类型。
