...
- [ ] 4.3 插件 source 改构建期组合并迁为 pane  ⛔ **已整体回滚**

  > **⛔ 本轮尝试已整体回滚 —— 它打红了既有的 aigc-canvas e2e 两条,根因未查清。**
  >
  > **验证成立的结论(下一轮可直接采信)**:插件确实**不需要跨 realm 传组件**。
  > 做法是把画廊 pane 的挂载抽成可复用函数 `mountCanvasPane(plugins, namespace)`,插件在
  > **构建期**与画布组件一起打包,在 pane 内交给既有的插件聚合逻辑接入。实测中 stickers 的
  > 第一个用例(贴纸闭环:工具轨 → 舞台置层 → Inspector → 拍平)**已跑通** ——
  > 原估的「需新建 guest 侧插件车道、体量近一个独立 spec」**不成立**。
  >
  > ★ 一处踩到的坑:`manifestId` 决定插件的**命名空间前缀**(工具/图层锚点都带它),
  > 起初写死成 "canvas-plugins",导致锚点名与 source 对不上、断言全红。已改为由调用方传入。
  >
  > **两处踩到的坑(下一轮别重踩)**:
  > 1. `manifestId` 决定插件的**命名空间前缀**(工具/图层锚点都带它)。起初写死成
  >    "canvas-plugins",导致锚点名与 source 对不上、断言全红。须由调用方传入。
  > 2. `stickers` 的风格迁移用例中,pane 内底部动作按钮被**宿主的浮动比例切换器**拦住点击。
  >    pane 内已有 56px 底部留白并写明了该问题,但带插件的动作排更高。
  >    ⚠ **试过加大到 80px,反而把无插件的 canvas 也打红** —— 留白不是越大越好,
  >    它改变了内部滚动容器的布局。
  >
  > **★ 回滚的真正原因**:回滚留白到 56px 后 aigc-canvas **仍然两条红**(同一个 edit 按钮
  > 点不动)⇒ 打红它的不是留白,而是**可复用改造本身**(挂载函数抽取 / 入口从 canvas.tsx
  > 改为 main.tsx / CanvasPanel 多了 extensions 分支)中的某一处。根因未查清,故整体回滚,
  > 不把一个打红既有测试的改动留在分支上。
  >
  > **下一轮的第一步**:只做「入口抽取」这一件事(canvas.tsx 导出 mountCanvasPane +
  > 新建 main.tsx 调用),**不加任何插件参数**,先确认 aigc-canvas 5/5 仍绿。
  > 绿了再加插件注入。一次只动一处,才能知道是哪一处打红的 —— 本轮三处一起动,才查不出来。
