...
- [x] 1.1 抽取/确认 `checkAllowlist` 可被 tool-kit 复用
...
- [x] 1.2 `gate.ts`：读门控 env + checkAllowlist 封装
...
- [x] 2.2 `install_extension` 工具：门控 + ctx.ui 进度 + pi.exec + 排队 reload
...
- [x] 2.3 `uninstall_extension` + `list_extensions` 工具
...
- [x] 3.1 option-mapper：forcedExtensionPaths 追加扩展工具入口
...
- [x] 3.2 pi-handler：下发扩展入口 + 门控 spawn env
...
本 spec 的实现已随「/install host 命令」整合线落地(旧 `/plugin` 摘除),但 `tasks.md` 未回勾
(spec.json 已是 `implemented`)。本次按 kiro 规范**逐任务用代码证据核实 + 跑测试取新鲜证据**后回勾。

| 任务 | 证据 |
|---|---|
| 1.1 / 1.2 | `tool-kit/src/extension-tools/gate.ts`:`DEFAULT_ALLOWLIST` / `checkAllowlist` / `toInstallArg` / `gateInstall` |
| 2.1–2.4 | `extension-manager.ts` 含 install/uninstall/list 三工具 + `/reload-runtime` 命令;`index.ts` barrel + `entry-path.ts` |
| 3.1 | 扩展工具入口已进强制注入清单 —— **注:承载方式已演进**,见下 |
| 4.1–4.4 | `/plugin` host 命令、内置斜杠命令、前端模态面板全仓零命中(已摘除) |
| 5.1 | `tool-kit/test/extension-tools/extension-manager.test.ts` |
| 5.2 | `e2e/node/extension-install.e2e.test.ts` |
| 5.3 | `e2e/browser/` 下扩展相关用例(`extension-ui-surfaces` / `install-host-command` 等) |

**新鲜证据**:扩展工具单测 **15 passed / 0 failed**(2026-07-24)。

### ⚠ 任务 3.2 的机制已被后续 spec 取代(如实记录)

3.2 原文是「pi-handler:下发扩展入口 + 门控 spawn env」。该机制已由
**[`runner-self-resolved-builtins`](../runner-self-resolved-builtins/design.md)(`ff9ba03`)退役** ——
`PI_WEB_EXT_TOOLS_ENTRY` 的主进程下发已移除,改为 runner 从自身安装树**自解析**
(`packages/server/src/runner/builtin-extensions.ts` 的 `BUILTIN_EXTENSIONS` 含
`{ id: "extension-tools", resolve: extensionManagerEntryPath }`)。

**能力结果不变**(扩展工具仍随每个会话强制注入),但**实现路径已不同**:此处标记完成指
「该能力已达成」,不表示 3.2 描述的下发代码仍然存在。旧 env 在过渡期仍被识别但不再是主来源。
