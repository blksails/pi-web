...
- [x] 1.1 建立会话层类型与错误定义
  ...
  - 从 `@blksails/pi-web-protocol` 引入帧/事件/响应类型别名,从 rpc-channel 引入 `PiRpcChannel`、从 agent-source-resolver 引入 `ResolvedSource`(仅类型导入)
...
- [x] 2. 核心:事件→UIMessage 纯函数翻译层
- [x] 2.1 (P) 实现翻译上下文 TranslationContext
...
- [x] 2.2 实现 translateEvent 纯函数与逐事件映射
...
- [x] 3. 核心:会话存储接口与内存实现
- [x] 3.1 (P) 实现 SessionStore 接口与 InMemorySessionStore
...
- [x] 4. 核心:PiSession 有状态外壳
- [x] 4.1 实现 PiSession 构造与事件广播
...
- [x] 4.3 实现 extension UI 挂起表与往返
  ...
  - _Boundary: pi-session.ts_
- [x] 4.4 实现生命周期:idle 回收、崩溃清理、stop 幂等、onClosed 去注册回调
...
- [x] 5. 集成:SessionManager 创建编排与优雅停机
- [x] 5.1 实现 createSession 编排与去注册接线
...
- [x] 6.1 集成测试:真实通道 + stub agent
  ...
  - _Requirements: 10.4_
  - _Depends: 4.3, 5.1_
- [x] 6.2 端到端测试:create→prompt→完整流→stats
