...
### Non-Goals
- 不改 `@blksails/pi-web-protocol` 契约与 `@blksails/pi-web-server` 路由(5 方法已解析、无回包)。
...
- `@blksails/pi-web-ui` 的 `PiChat` / `PiChatBasic` / `PiChatPro`(别名)导出收敛。
...
- 上游:`@blksails/pi-web-protocol`(`RpcExtensionUIRequest` 类型,只读消费)、`@blksails/pi-web-react`(`ControlStore` / `useExtensionUI` / 连接)、`@blksails/pi-web-ui` 既有元件与 `cn`。
