...
- [x] 1.1 ControlStore 推送类分流为 ambient 状态
  ...
  - 观察完成态:`pnpm --filter @blksails/react typecheck` 通过;给定五类推送帧后快照对应切片正确变化(置/替换/删除/计数),且交互类帧仍只进队列(由 1.1 自带或 4.1 单测断言)
...
- [x] 1.2 useExtensionUI 暴露 ambient 状态并导出类型
  ...
  - 观察完成态:`pnpm --filter @blksails/react test typecheck` 通过;外部可从 `@blksails/react` 导入新 ambient 类型与扩展后的结果类型
...
- [x] 3.1 富组件收敛为默认 PiChat,最小组件改名,保留废弃别名
  ...
  - 观察完成态:`pnpm --filter @blksails/ui test typecheck` 通过;可从 `@blksails/ui` 导入 `PiChat`(富)/`PiChatBasic`(最小)/`PiChatPro`(别名),三者渲染不报错
...
- [x] 4.1 (P) ControlStore 分流单测
  ...
  - 观察完成态:`pnpm --filter @blksails/react test` 通过且覆盖上述分支
...
- [x] 4.2 (P) 三个展示元件单测
  ...
  - 观察完成态:`pnpm --filter @blksails/ui test` 中三元件用例通过
...
- [x] 4.3 PiChat 集成测试(渲染面 + 不阻塞 + 收敛)
  ...
  - 观察完成态:`pnpm --filter @blksails/ui test` 中 PiChat 集成用例通过,覆盖各面与不阻塞断言
...
- 1.2 连带:`UseExtensionUIResult` 新增必填 ambient 字段后,ui 包共享夹具 `packages/ui/test/fixtures/mock-session.ts` 的 extensionUI mock 需提供 ambient 默认值(空数组/空对象/undefined + dismissNotification no-op),否则 `pnpm --filter @blksails/ui typecheck` 会红。已在 ui 元件任务前由父统一修复夹具。
