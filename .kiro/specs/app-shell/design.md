...
**Purpose**:本特性交付 pi-web 的**整站闭环装配**——一个可直接运行、可部署、可作分层参考实现的 Next.js 应用。它把已就绪的各层装配起来:把 `http-api` 的 `createPiWebHandler` 挂到 `app/api/sessions/**` 的 REST + SSE 路由;在 `app/page.tsx` 用 `@pi-web/react` 的 hooks 建立指向本站 API 的连接并渲染 `@pi-web/ui` 的 `<PiChat>`;提供 agent 源选择器(按 `agent-source-resolver` 的输入形状传源);从 `.env.local` 注入默认 provider/model/agent 源/工作区/密钥;附一个用 `defineAgent` 写的示例 agent 与 `.pi/` 资源样例供端到端验证。
...
- **上游 spec(运行时)**:`http-api` 的 `createPiWebHandler` 与会话依赖装配工厂;`@pi-web/react` 的 `usePiSession`/`usePiControls`/`useExtensionUI`/`PiProvider`/`PiTransport`/`createPiClient`;`@pi-web/ui` 的 `<PiChat>` 及导出;`@pi-web/agent-kit` 的 `defineAgent`(示例 agent 用);经上游间接的 `@pi-web/protocol` 类型。
- **外部运行时**:Next.js 15(App Router,Node runtime)、React 18+、AI SDK v5 + AI Elements + shadcn/Tailwind v4(经 `@pi-web/ui` 装配)。
...
|-------|------------------|-----------------|-------|
| Frontend / CLI | TypeScript strict;Next.js 15(App Router/RSC);React 18+ | 应用骨架、页面、源选择器视图;经 `@pi-web/ui` 渲染 `<PiChat>` | 浏览器 + RSC;主题经 shadcn CSS 变量 |
...
| Messaging / Events | SSE(经 handler 的 `text/event-stream`);前端经 `@pi-web/react` transport 消费 | 流式聊天主链路 | shell 不编码 / 不解码 SSE |
...
| app/page.tsx | app | 选源 → 建会话 → 渲染 PiChat + 控制 + 弹窗 | 1.3,1.5,4.2,4.3,4.5,5.1,5.2,6.x,7.1 | @pi-web/react (P0), @pi-web/ui (P0), agent-source-picker (P0) | Service, State |
...
| components/agent-source-picker.tsx | components | 源输入 + 提交 + 进行 / 错误态 | 1.5,4.1,4.4,4.5 | @pi-web/react (P1) | Service, State |
| examples/hello-agent/index.ts | examples | defineAgent 示例(工具 + 思考 / MD,e2e fixture) | 8.1,8.2,8.4 | @pi-web/agent-kit (P0) | Service |
...
**Contracts**: Service / State。**Implementation Notes**:`page.tsx` 不直接发起 fetch/SSE(经 `@pi-web/react`);`AgentSourcePicker` 仅产出 `source` 字符串与提交意图。Validation:页面渲染冒烟(`route.integration.test.ts` 或单独渲染测试)+ e2e 全覆盖交互。
...
- **核心消费契约**:REST/SSE 端点与帧形状一律来自上游(`createPiWebHandler` + `@pi-web/protocol`),本 spec 不定义、只委托与连接。
