...
- **Findings**:
  ...
  - ui-components:`<PiChat session controls extensionUI slots showControls className />`,内部用 `useChat(transport)` + AI Elements;`usePiSession`/`usePiControls`/`useExtensionUI` 来自 `@blksails/pi-web-react`。
...
| 在 page 内自建 fetch/SSE | 不用 `@blksails/pi-web-react`,页面直接消费 SSE | 控制力强 | 重实现 transport/hooks,严重越界 | 明确排除 |
