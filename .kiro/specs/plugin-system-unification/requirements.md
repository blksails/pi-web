...
### Requirement 12: 项目 skill 加载与 `skill:` 命令暴露（增量，调查中）

**Objective:** 作为插件作者，我希望项目级 `.pi/skills` 的技能被加载、并（如 SDK 设计）以
`skill:<name>` 出现在命令补全，这样我的插件技能在 web 端可见可用。

> 背景（调查结论，见 evidence.md）：实测 pi-web 下**任何项目 skill 都不出现为 `skill:` 命令**
> （连已知的 `pi-probe-agent` 自带探针 skill 也不出）。已排除 app 层全部成因（路径已在 `.pi/skills`、
> `def.skills` 未 override、`--no-skills` 默认不注入、`collectSkillEntries` 递归命中、`enabled` 默认 true、
> `projectTrusted` 为真、RPC 模式 get_commands **确含** `getSkills()→skill:`、非时序）。`resourceLoader.getSkills()`
> 仍为空。根因落在 SDK/runner 子进程边界,需**捕获 runner 子进程 stderr 的专门调试**进一步定位
> （dev 日志未接住临时诊断;`module.createRequire failed parsing argument` 警告为可疑线索:疑似
> custom runRpcMode 加载异常或回退路径）。

#### Acceptance Criteria
1. When 一个 agent source / 已装插件在 `.pi/skills`（自运行）或包根 `skills/`（被安装）含 `SKILL.md`，the 系统 shall 加载该技能（注入系统提示 / 可被模型使用）。
2. When 技能已加载，the 系统 shall 在 `GET /sessions/:id/commands` 以 `skill:<name>`（source `skill`）暴露之，并出现在命令补全。
3. The 调查 shall 先定位"`getSkills()` 为空"的确切原因（runner 模式 / SDK 资源加载），再决定修法（不盲改 runner）。
