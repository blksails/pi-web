...
### Requirement 15: registerCommand 命令是动作——无气泡、不进消息历史（增量，取代 R13 + 回退 R11 气泡）

**Objective:** 作为用户，我希望 `registerCommand` 注册的扩展斜杠命令（`/review`、`/plugin` 等工具/动作命令）
**不在转录区留任何消息痕迹**——既不显示用户气泡（实时），也不进消息历史（冷恢复）——因为它们是**动作**而非对话，
反馈只走 ctx.ui；而 **skills / prompt template 是对话**，照常进历史、有气泡（保留 R14）。

> 决策来源（用户）：用户审视 R13（把命令标记塞进历史）后判定方向相反——registerCommand 命令是动作，不该进历史；
> 实时也不应有气泡。skills/template 经展开成 prompt 触发 turn，是真实对话，进历史有气泡（R14 折叠显示）。
> 取舍（用户已选「简单回退」）：回退 R11 对扩展命令的「正常 send」改动 → fire-and-forget；其代价是**触发 turn 的**
> registerCommand 命令那一轮不实时渲染（R11 原修的不一致），但按「命令是动作」的定调可接受。

#### Acceptance Criteria
1. When 提交一个 `registerCommand` 扩展命令（`source==="extension"` 的斜杠命令），the 前端 shall 经 `client.prompt` **fire-and-forget** 投递（不经 useChat.sendMessage），**不渲染用户气泡**、输入区即时复位。
2. The registerCommand 命令 **shall not** 被持久化为消息历史；冷恢复后 `GET /messages` **shall not** 含该命令的任何痕迹。
3. The registerCommand 命令的 ctx.ui 反馈（notify/status/widget）shall 经临时点亮的「仅控制」流（`armExtControlStream`）渲染，即使是**无 webext** 的纯 registerCommand 扩展（`needsIdleControl` 否则为 false）。
4. The skills / prompt template（非 registerCommand）**shall not** 命中此分支：仍走正常 send（doSend）→ 进历史、有气泡（R14 折叠显示），不受本需求影响。
5. The `/plugin`、`/reload-runtime` 命令 shall 仍触发 webext 重载（双路生效路②）。
6. The 行为 shall 经浏览器 e2e 验证：提交 `/review` → ctx.ui notify 出现 + 转录区无 `/review` 气泡 → 冷恢复后转录区仍无 `/review`。

> **R13（已撤销，保留记录）**：原「纯扩展命令的历史持久化（落地 R11-AC4）」把 registerCommand 命令以 `piweb.command`
> session 条目持久化并在 `GET /messages` 合并 surfacing 为用户气泡。用户判定方向相反（见 R15），**R13 已整体撤销**
> （runner seam / server surfacing / stub sentinel / 测试 / e2e 全部移除）。以下 R13 原文仅留作设计演进记录。

### Requirement 13（已撤销，被 R15 取代）: 纯扩展命令的历史持久化（增量，落地 R11-AC4）
...
### Requirement 14: skill 命令历史显示与实时一致（增量，用户实测发现）

**Objective:** 作为用户，我希望 `/skill:<name>` 命令在**历史回放**里和**发送当下**显示一致（都显示短命令
`/skill:<name>`），这样刷新后不会把一大段展开的 SKILL.md 正文当作我发的消息显示出来。

> 背景（用户截图实证）：`/skill:<name>` 非扩展命令，pi SDK `AgentSession._expandSkillCommand` 把它**展开成
> `<skill name="…">…</skill>` 块当 prompt** 送模型（展开内容进 LLM 上下文是 skill 的本意，正确且应保留）。
> 但 `get_messages` 历史取出的 user 文本即该展开块：**实时**乐观气泡显示 `/skill:code-review-skill`（短），
> **历史**却显示整段展开正文（References/Code Review/何时触发/步骤…）——刷新后才暴露的显示不一致。
> 与 R13 不同：R13 是"工具命令 0 持久化"，R14 是"skill 已持久化但展示形态不一致"。

#### Acceptance Criteria
1. When 历史回放渲染一条 user 消息，且其文本是 skill 展开块（`^<skill name="…" location="…">…</skill>`），the 系统 shall 把它折叠回 `/skill:<name>`（有 args 时为 `/skill:<name> <args>`）再渲染。
2. The 折叠 shall 仅作用于**前端历史显示**（`agent-message-to-ui`）；**shall not** 改写 server message log——模型上下文仍是展开内容（保留 skill 进上下文的本意）。
3. The 折叠 shall 不误伤普通用户文本（仅匹配稳定的 `<skill name="…" location="…">…</skill>` 形态；畸形/无闭合块原样保留，安全降级）。
4. The 修复 shall 经单测（string/数组 text/带 args/普通文本不误折叠/畸形降级）+ 浏览器 e2e（提交 `/skill:` → 实时短命令 → 冷恢复仍折叠为短命令、不显示展开正文）验证。
