...
> 语言:zh。本计划覆盖 `@blksails/pi-web-react` 全部组件与硬性测试(单元 / 组件-集成 / e2e)。
> 依赖方向:`@blksails/pi-web-protocol` + `http-api` 契约 + AI SDK ← `@blksails/pi-web-react`。仅浏览器环境,经 HTTP/SSE 消费,不依赖后端内部对象。

- [x] 1. 基础设施:包脚手架与测试环境
- [x] 1.1 初始化 `@blksails/pi-web-react` 包与构建/类型配置
  - 创建 `packages/react/package.json`(name `@blksails/pi-web-react`、`sideEffects:false`、deps `@blksails/pi-web-protocol`/`ai`、peerDeps `react`/`@ai-sdk/react`)、`tsconfig.json`(strict、DOM lib、ES2022)
  - 建立 `src/` 目录骨架(client / sse / transport / hooks / provider / version)与空 `index.ts` 聚合导出占位
  - 观察完成:`tsc --noEmit` 在空骨架下通过,包可被 workspace 解析到 `@blksails/pi-web-protocol` 类型
  - _Requirements: 9.1_
...
- [x] 2.3 (P) 实现 uiMessageChunk → AI SDK UIMessageChunk 解码映射
  - 以 `@blksails/pi-web-protocol` 的 `UiMessageChunkSchema`/`DataPartSchema` 为来源,映射 text(start/delta/end)、reasoning(start/delta/end)、tool(input/output-available)、`data-pi-*` data-part 为对应 AI SDK chunk
...
- [x] 2.6 (P) 实现 protocolVersion 兼容判定
  - 以 `@blksails/pi-web-protocol` 的 `protocolVersion` 为唯一基准,对 SSE 帧/REST 响应携带的版本做兼容判定;不兼容 → `PiProtocolVersionError`(复用 2.1)
...
- [x] 3.1 实现 createPiClient 全部端点方法
  - 基于 2.4/2.5,实现 `createSession`/`prompt`/`steer`/`followUp`/`abort`/`setModel`/`setThinking`/`uiResponse`/`getState`/`getStats`/`getMessages`/`getCommands`/`deleteSession`,形状取自 `@blksails/pi-web-protocol` REST DTO 与 `http-api` 端点路径
...
- [x] 7.1 完成 index.ts 聚合导出
  - 从各层 re-export `PiTransport`/`createPiClient`/`usePiSession`/`usePiControls`/`useExtensionUI`/`PiProvider` 及公开类型,作为 `@blksails/pi-web-react` 唯一导入面
  - 观察完成:外部按 `import { PiTransport, usePiSession } from "@blksails/pi-web-react"` 可解析;`tsc --noEmit` 通过
  - _Requirements: 9.1_
