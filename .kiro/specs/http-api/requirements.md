...
本 spec **消费而非重定义**上游契约:REST DTO、SSE 帧 schema、`protocolVersion` 取自 `@pi-web/protocol`;`PiSession` API、`SessionStore`、`subscribe()`、命令转发取自 `session-engine`。权威设计见 `PLAN.md` §3.3(端点表)、§13.2(`createPiWebHandler`/协议)、§11.5(SSE 反代)、§14.1③(控制面/数据面分离)、§13.4(可插拔鉴权点)。
...
- **In scope**(本 spec 拥有):
  ...
  - 每个处理器的请求边界校验(用 `@pi-web/protocol` DTO `safeParse`)与统一错误码/错误响应体。
...
1. When 收到 `POST /sessions` 且请求体经 `@pi-web/protocol` 的建会话 DTO 校验通过(含必填 `source`,可选 `cwd`/`model`/`env`),the http-api shall 经上游创建会话并返回 `{ sessionId }`(`200/201`)。
...
5. The http-api shall 对查询端点的响应体形状以 `@pi-web/protocol` 的对应响应 DTO 为准,不重定义形状。
...
1. The http-api shall 在响应(REST 响应头/响应体与 SSE 帧)中携带来自 `@pi-web/protocol` 的 `protocolVersion`。
2. When 客户端在请求中声明了不兼容的 `protocolVersion`,the http-api shall 返回明确的版本协商错误(如 `426`/`400` 类)而非静默继续。
3. The http-api shall 以 `@pi-web/protocol` 导出的 `protocolVersion` 为唯一版本来源,不自定义版本号。
