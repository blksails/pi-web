...
**Impact**:把 `PLAN.md` §3.3 的 Route Handlers、§13.2 的 `createPiWebHandler`、§11.5 的 SSE 反代要点、§14.1③ 的"网关只转发、状态在通道背后"收敛为边界清晰、可单测的 HTTP 层。本 spec **消费**上游契约(`@blksails/protocol` 的 DTO/SSE 帧/`protocolVersion`、`session-engine` 的 `PiSession` API),不重定义。

### Goals

- 提供 `createPiWebHandler(opts)`:标准 Web Fetch 处理器,内部方法+路径路由到端点处理器;可挂 Next.js/Hono/Express(adapter)。
- 实现全部 REST 端点(创建/命令转发/查询/删除)与 SSE 流端点,边界处用 `@blksails/protocol` DTO `safeParse` 校验,失败映射统一错误码。
...
- 请求边界校验(用 `@blksails/protocol` DTO `safeParse`)与统一错误响应结构 + HTTP 状态码映射。
...
- **上游 spec(运行时)**:`@blksails/protocol`(REST DTO schema、`SseFrameSchema`、`protocolVersion`,单一事实来源 + 边界校验);`session-engine` 的 `SessionManager`/`SessionStore` 接口与 `PiSession` 对外契约(`subscribe`/命令转发方法/`stop`/`getCachedState`)与错误类型(`SessionStoppedError`/`SessionNotFoundError`/`UnknownExtensionUIError`/`MissingInputError`)。
...
- `@blksails/protocol` 的 REST DTO / `SseFrameSchema` / `protocolVersion` 承载约定变更。
...
| Messaging / Events | SSE(`text/event-stream`);消费 `PiSession.subscribe()` 的 `SseFrame` 流 | 把帧编码为 event-stream,心跳防断,`id:` 重连 | 帧形状由 `@blksails/protocol` 定义 |
...
├── http/
│   ├── validate.ts            # 用 @blksails/protocol DTO safeParse 校验请求体/参数;返回校验错误或 typed body
...
- 无(greenfield 新模块)。若 monorepo 已存在 `package.json`,需将 `@blksails/protocol` + `session-engine` 模块与 `vitest` 纳入依赖——接线随仓库初始化处理,本 spec 创建模块自身文件与测试。宿主挂载示例(Next.js Route Handler / Hono / Express adapter)作为文档/示例随上层装配,不属本模块运行时文件。
...
| 7.3 | protocolVersion 单一来源 | version.ts | `@blksails/protocol` | — |
...
| sse/sse-encoder.ts | sse | 帧→event-stream 文本编码 | 5.2,5.5,6.1,6.4,7.1 | @blksails/protocol (P0) | Event |
...
| http/validate.ts | http | protocol DTO safeParse 校验 | 2.2,3.3,4.5 | @blksails/protocol (P0) | Service |
...
| http/version.ts | http | protocolVersion 握手/承载 | 7.1,7.2,7.3,6.4 | @blksails/protocol (P0) | Service |
...
**Summary-only**:各端点处理器在边界用 `validate.ts`(`@blksails/protocol` DTO `safeParse`)校验请求体/参数,失败→400(附字段路径);通过后转发到注入的 `SessionManager.createSession` 或 `PiSession` 的命令/查询方法,把结果用 `responses.ts` 序列化为带 `protocolVersion` 的 `Response`;引擎抛出的已知错误经 `error-map.ts` 映射状态码。`create-session` 在停机标志置位时返回 503(Req 2.5);`delete-session` 触发 `PiSession.stop()` 并依赖上游经 `onClosed` 从 store 移除。Contracts: API。覆盖 Req 2.x、3.x、4.x。
...
> 所有 DTO 形状取自 `@blksails/protocol`;命令负载子类型取自其 `RpcCommand`/REST DTO。命令端点不重定义负载形状。
>
...
**Responsibilities & Constraints**
- 输入 `SseFrame`(`@blksails/protocol`,`uiMessageChunk` 与 `control` 两类)+ 单调帧序号;输出 `text/event-stream` 文本块:`event:`(可选,按帧类别)、`data: <json>`、`id: <seq>`,以空行分隔(Req 5.2/6.1)。
...
**Summary-only**:
- `validate.ts`:对请求体/参数调用 `@blksails/protocol` DTO `safeParse`,失败返回带字段路径的校验错误,成功返回 typed body(Req 2.2/3.3/4.5)。
...
- `version.ts`:以 `@blksails/protocol` 的 `protocolVersion` 为唯一来源(Req 7.3),承载到响应/帧(Req 7.1),请求声明不兼容版本时产出 426/400 协商响应(Req 7.2)。
...
- **核心对外契约**:REST 端点(请求/响应 DTO)与 SSE 帧——形状一律取自 `@blksails/protocol`(`CreateSessionRequest`/各命令 DTO/各响应 DTO/`SseFrame`),本 spec 不重定义(单一事实来源,Req 4.5)。
- **序列化格式**:REST 走 JSON;SSE 走 `text/event-stream`(`data:`/`event:`/`id:` 行 + 心跳注释帧)。
- **版本**:`protocolVersion`(`@blksails/protocol`)承载于 REST 响应头/体与每个 SSE 帧,供前后端协商(Req 7.x);唯一来源,网关不自定义。
...
- **版本**(`version.test.ts`):响应/帧承载 `protocolVersion`;不兼容请求→协商错误;来源为 `@blksails/protocol`。(7.1,7.2,7.3)
