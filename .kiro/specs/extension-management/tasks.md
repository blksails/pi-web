...
- [ ] 1.1 建立 extension-management 模块脚手架与本层共享类型
  ...
  - 对外 DTO(扩展列表 / 安装请求)优先从 `@pi-web/protocol` 导入;上游未导出时本地按一致命名定义并注明对齐来源;命令清单 DTO 归 `http-api`,本层不定义
...
- [ ] 2.2 (P) 实现 pi 命令参数装配 + 非交互 git env
...
- [ ] 2.3 (P) 实现信任落地映射(trustPolicy → 信任片段)
...
- [ ] 4.1 (P) 实现列出已安装扩展端点(GET /extensions)
...
- [ ] 4.2 实现安装端点(POST /extensions,治理编排)
...
- [ ] 4.3 实现卸载端点(DELETE /extensions/:id)
...
- [ ] 4.4 实现会话重载端点(POST /sessions/:id/reload)
...
- [ ] 5.1 实现路由注册表并经 http-api routes? 注入接缝接入
...
- [ ] 5.2 fixture 扩展集成测试(install→list→消费 http-api commands→remove)
...
- [ ] 6.1 e2e:装扩展→reload/新会话→命令生效→prompt 调用
