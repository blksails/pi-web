...
- 会话引擎、agent 源解析、runner、协议、`@blksails/pi-web-ui` 组件库的任何行为；autostart 仅在 app-shell 装配层（`chat-app`/`page`）做极小接线，复用既有 resume 模式的「跳过 picker 直接进会话」机制。
...
- app shell 经 `app/api/sessions/[[...path]]/route.ts` → `lib/app/pi-handler.ts` 的 `getHandler()`（globalThis 单例）服务会话；前端经 `@blksails/pi-web-react` 连本站 API。
