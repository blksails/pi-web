...
- **传输/隔离/存储用接口隔开**:`PiRpcChannel`、`agentHostProvider`、`SessionStore`、`BlobStore`(附件对象存储,LocalFs→S3)是为未来(e2b/edge/device、对象存储)预留的接缝,按接口写、后端经配置切换。附件能力按 L0 存储 / L1 引用 / L2 投影(resolve) / L3 context 闸门分层,模块在 `@blksails/server` 的 `attachment/`(存储)与 `attachment-bridge/`(tool 桥接)。
- **协议是稳定契约**:`@blksails/protocol` 的类型/schema 改动需语义化版本;SSE 帧带 `protocolVersion`。
