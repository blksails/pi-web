...
| `@blksails/pi-web-core` | **headless 内核**:会话引擎、传输抽象、框架无关 HTTP、宿主契约端口、workspace/capability/config-domain。依赖只有 `logger` / `protocol` / `zod` / `tool-kit`;agent SDK 仅 optional peer 且**只以类型引用**。**不含** e2b / pg / MCP SDK / hono。 |
| `@blksails/pi-web-server` | 装配层 + adapters + runner 实现,并对 `pi-web-core` 做**兼容 re-export**(包名与 6 个子路径导出逐字不变)。adapters 绑定具体外部系统:e2b 传输、postgres store、MCP 探测、ai-gateway / llm-gateway、auth / identity、镜像烘焙、包安装。依赖 pi SDK、e2b、jiti、pg、ws、MCP SDK。 |
...
**依赖方向铁律**:`logger` / `protocol` 在最内层,被几乎所有包依赖;`protocol ← 所有`;
`core` 只依赖 `protocol` + `logger` + `zod` + `tool-kit`;`server` 依赖 `core`(反向不成立);
`react`/`ui` 与后端解耦。写新包/新依赖前先确认不制造反向或环形依赖。

后端内部另有一层**机械守着**的分层(名册 `packages/core/test/tiering/module-roster.ts`):

    neutral  ←  core  ←  { runner, adapters }  ←  assembly

跨层反向依赖由 `dependency-guard.test.ts` 拦截(含跨包深路径与动态 import);
新增 `src/` 顶层模块**必须**在名册里表态,否则守卫报红 —— 一个默认无人管的新模块正是边界腐化的入口。
...
- **传输/隔离/存储用接口隔开**:`PiRpcChannel`、`agentHostProvider`、`SessionStore`、`BlobStore` 是为未来(e2b/edge/device、对象存储)预留的接缝,按接口写、后端经配置切换。附件能力按 **L0 存储 / L1 引用 / L2 投影(resolve) / L3 context 闸门**分层,模块在 `attachment/`(存储)与 `attachment-bridge/`(tool 桥接)。
