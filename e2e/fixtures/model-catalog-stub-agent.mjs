#!/usr/bin/env node
/**
 * model-catalog-stub-agent — fork of `lib/app/stub-agent-process.mjs` for
 * spec multi-gateway-providers, task 8.2 (fix round; Req 11.3).
 *
 * ## Why a dedicated fork (not the shared production stub)
 *
 * The production stub's `get_available_models` returns a hard-coded constant
 * (`AVAILABLE_MODELS`), completely decoupled from `<agentDir>/providers.json` —
 * so no browser e2e driven by the default stub can observe "a custom provider's
 * model reaches the SESSION model selector" (Req 11.3). That production chain is:
 * `runner.ts` → `composeModelSources()` → `host-assembly/model-sources.ts`
 * `registerBuiltinModelSources()` → `model-source-registrar` → synthesized
 * `ModelRegistry` → pi SDK rpc-mode's `get_available_models` (one line:
 * `session.modelRegistry.getAvailable()`, see
 * `packages/runner/node_modules/@earendil-works/pi-coding-agent/dist/modes/rpc/
 * rpc-mode.js`) → `GET /api/sessions/:id/models` → `use-models.ts` →
 * `model-selector.tsx`. This fork exercises the SAME production entry point
 * (`registerBuiltinModelSources`) the real runner calls, so the wiring under
 * test is genuine, not reimplemented.
 *
 * ## Fidelity boundary (do not overstate what this proves)
 *
 * This fixture recreates ONLY the one line pi SDK's rpc-mode uses
 * (`modelRegistry.getAvailable()`) against a registry assembled the same way
 * `option-mapper.ts:111-137` assembles it (`listModelSources()` +
 * `getSharedModelServicesFactory()(agentDir)` + per-source
 * `resolveSpecFromEnv`/`register`). It does **not** run a real runner process,
 * `buildRuntimeFactory`, or the pi SDK's session/agent loop — those are covered
 * elsewhere:
 *  - `packages/runner/test/runner/option-mapper-model-source.it.test.ts` —
 *    the synthesis in `option-mapper.ts` itself;
 *  - `packages/runner/test/runner/model-source-registrar.it.test.ts` —
 *    the registrar contract;
 *  - `packages/server/test/host-assembly/custom-provider-session-source.it.test.ts` —
 *    `registerBuiltinModelSources()` end to end against a real `ModelRegistry`.
 * This fixture's job is narrowly to prove those pieces are actually reachable
 * from the SESSION model-list RPC path in a browser-observable way.
 *
 * Everything else (RPC skeleton, prompt/turn handling, persistence, commands,
 * routes, etc.) is copied byte-for-byte from the production stub so existing
 * frame behavior (readiness handshake, `set_model`, `PI_WEB_DEFAULT_MODEL=
 * stub-model`, …) is unaffected — see that file's header for the full
 * behavior contract. The ONLY change is `get_available_models`: its response
 * is now the UNION of the original deterministic `AVAILABLE_MODELS` (unchanged,
 * so `stub-model`/`stub-opus`/`stub-gpt` keep existing) with whatever
 * `registerBuiltinModelSources()` resolves from the real env/`providers.json`
 * (egress / ai-gateway / custom-providers — in this e2e project, only
 * custom-providers is ever configured). See `buildCatalogModels()` below.
 */
...
// This fixture lives at <repo>/e2e/fixtures/model-catalog-stub-agent.mjs. Resolve
// the repo root from its own location (same pattern as
// attachment-tool-bridge-stub.mjs's EXAMPLE_TOOL_PATH).
const FIXTURE_DIR = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(FIXTURE_DIR, "..", "..");
const MODEL_SOURCE_REGISTRAR_PATH = path.join(
  REPO_ROOT,
  "packages",
  "runner",
  "src",
  "runner",
  "model-source-registrar.ts",
);
// `jiti`'s ESM entry, resolved by absolute path via `packages/server`'s OWN
// `node_modules/jiti` symlink (packages/server declares `jiti` as a real
// dependency — the repo ROOT `package.json` does not, so a bare `import("jiti")`
// from this file, which only sees root `node_modules`, would throw
// `ERR_MODULE_NOT_FOUND`, same as `@blksails/pi-web-runner` below).
const JITI_MJS_PATH = path.join(REPO_ROOT, "packages", "server", "node_modules", "jiti", "lib", "jiti.mjs");
...
/**
 * 为**真实存在**的附件签出分发 URL(与主进程同 DIR/同 secret,故主进程可校验)。
 *
 * 仅 B 档 register 用:那条路径上的 att_ 是浏览器真上传出来的。附件 env 未下发、
 * 附件不存在或签名失败一律返回 undefined,由调用方降级到 data: URI —— stub 不因此崩。
 */
let stubAttachmentStore;
let stubAttachmentStoreTried = false;
async function presignStubAttachment(id) {
  if (id === "") return undefined;
  if (!stubAttachmentStoreTried) {
    stubAttachmentStoreTried = true;
    try {
      const api = await loadServerStoreApi();
      stubAttachmentStore = api.createChildAttachmentStore(process.env);
    } catch {
      stubAttachmentStore = undefined;
    }
  }
  if (stubAttachmentStore === undefined) return undefined;
  try {
    // head 先确认存在:对不存在的 id 签名会得到一个必然 404 的 URL,反而比 data: URI 更糟。
    await stubAttachmentStore.head(id);
    return await stubAttachmentStore.presignUrl(id);
  } catch {
    return undefined;
  }
}

/** aigc-canvas:构造一张 stub 画廊资产(仅 att_ 引用 + 展示 URL,无二进制)。 */
function stubCanvasAsset(id, over = {}) {
  return {
    attachmentId: id,
    // 缺省用 512 SVG data URI(A 档产物在 stub 下没有真实字节);B 档 register 会用
    // `over.displayUrl` 覆盖成真实签名 URL(见 presignStubAttachment)。
    displayUrl: over.displayUrl ?? stubCanvasImage(id),
    mimeType: "image/png",
    name: `${id}.png`,
...
/**
 * Load the Postgres/fs session-store factory from @blksails/pi-web-adapters.
 *
 * ★ It used to come from the server barrel; the adapters extraction
 * (spec adapters-package-extraction, task 5.1) removed that forwarding.
 * ⚠ This file is **.mjs — no typecheck sees it**. Had this call site been missed,
 * every typecheck would still pass and the stub would only fail at runtime
 * ("store init failed") in the node-e2e persistence suites.
 */
...
/**
 * Load the public protocol barrel only when the ext-askq seam is exercised.
 *
 * The production stub is spawned with `--import jiti/register` from the server
 * package directory, allowing the workspace package's TypeScript entrypoint to
 * execute. Keeping this lazy preserves the standalone JSONL seams that launch
 * the stub without jiti and never use AskUserQuestion.
 */
async function loadAskQuestionCodec() {
  const mod = await import("@blksails/pi-web-protocol");
  return mod.default ?? mod;
}

// ── model-catalog-stub-agent 专属:真实模型源装配(spec multi-gateway-providers,
// 任务 8.2 修复轮,Req 11.3)─────────────────────────────────────────────────
//
// ★ 实测坑(不要在不理解此处的情况下"简化"回 native `import()`):`--import
//   jiti/register`(本进程的全局 loader hook,由 `stubSpawnSpec` 固定传入)转译
//   `.ts` 文件时,该文件自身的静态 `import` 语句被转成 CJS 风格调用,经**该文件专属
//   的一个内部 jiti 实例**(`jiti-hooks.mjs` 的 `_wrapSource` 为每个被加载模块各建
//   一个)解析——这与本文件用 Node 原生 `import()` 直接加载同一份 `.ts` 源码走的是
//   两条独立通道。实测验证:两次分别加载 `model-source-registrar.ts`(一次原生
//   `import()`,一次经 `host-assembly/model-sources.ts` 内部的静态 import)会得到
//   两个**互不共享状态**的模块实例——`registerBuiltinModelSources()` 登记的模型源,
//   从另一条通道读到的 `listModelSources()` 里完全看不见(`registrars` 数组为空)。
//   解法:改用 `jiti` 自己的 `createJiti().import()` API(与 `runner-bootstrap.mjs`
//   同惯例)加载**两侧**——它们的求值最终都归并到 Node 的模块级缓存(按解析后的
//   绝对路径去重),两次 `jiti.import()` 调用因此共享同一个 `model-source-
//   registrar.ts` 实例、同一个 `registrars` 数组。已用独立脚本反复验证:混用原生
//   `import()` 与 `jiti.import()`不生效,必须两次加载都走 `jiti.import()`。

/** Lazily-constructed single `jiti` instance shared by both loads below (see note above). */
let jitiInstance;
async function getJiti() {
  if (jitiInstance === undefined) {
    const { createJiti } = await import(JITI_MJS_PATH);
...
/**
 * Load the SAME assembly module the real runner calls
 * (`packages/server/src/host-assembly/model-sources.ts`, exported as
 * `@blksails/pi-web-server/host-assembly/model-sources.js`) to get
 * `registerBuiltinModelSources`.
 */
async function loadModelSourcesAssembly() {
  const jiti = await getJiti();
  const mod = await jiti.import("@blksails/pi-web-server/host-assembly/model-sources.js");
  return mod.default ?? mod;
}

/**
 * Load the runner's model-source registrar by absolute TS-source path (see
 * `MODEL_SOURCE_REGISTRAR_PATH` comment above for why a bare package specifier
 * does not resolve from this file) for `listModelSources` /
 * `getSharedModelServicesFactory` — the SAME contract `option-mapper.ts` reads
 * from (`buildRuntimeFactory`, lines ~111-137). Shares module state with
 * `host-assembly/model-sources.ts`'s own internal import of the same file
 * ONLY because both go through `jiti.import()` on the SAME `jiti` instance
 * (see the note above the instance getter).
 */
async function loadModelSourceRegistrar() {
  const jiti = await getJiti();
  const mod = await jiti.import(MODEL_SOURCE_REGISTRAR_PATH);
  return mod.default ?? mod;
}

/** Registered once per process (registerModelSource dedupes by sourceId anyway; this just avoids repeat work). */
let builtinModelSourcesRegistered = false;

/** Minimal structured-logger shape the registrar contract requires (`ModelSourceLogger`). */
const modelSourceLog = {
  info(msg, data) {
    process.stderr.write(
      `model-catalog-stub-agent: ${msg}${data ? " " + JSON.stringify(data) : ""}\n`,
    );
  },
};

/**
 * Resolve the SAME set of models the real runner would expose to a session via
 * `registerBuiltinModelSources()` — egress / ai-gateway / custom-providers,
 * whichever declare a spec under the current env / `<agentDir>/providers.json`.
 * In this e2e project only custom-providers is ever configured (no
 * PI_WEB_CLOUD_LOGIN_… / PI_WEB_GATEWAYS env here), but the call goes through the
 * real registrar contract rather than special-casing custom-providers, so any
 * future source gains the same session-side observability for free.
 *
 * Returns `[]` (not throwing) when no source declares a spec or when
 * `PI_WEB_AGENT_DIR`/`PI_CODING_AGENT_DIR` is unset — mirrors the production
 * "nothing to inject → SDK default, no-op" fail-soft posture.
 *
 * ★ Note: `getAvailable()` also returns any REAL built-in provider the host
 * machine happens to have credentials for (`~/.pi/agent/auth.json` under
 * `agentDir`, or config-value env vars) — harmless noise for this e2e (the
 * assertions match on an exact, randomly-generated provider+model id, so
 * pre-existing/real credentials never collide), but worth knowing if this
 * response ever looks larger than expected while debugging.
 */
async function buildCatalogModels() {
  const agentDir = process.env.PI_WEB_AGENT_DIR ?? process.env.PI_CODING_AGENT_DIR;
  if (agentDir === undefined || agentDir.length === 0) return [];

  const { registerBuiltinModelSources } = await loadModelSourcesAssembly();
  if (!builtinModelSourcesRegistered) {
    registerBuiltinModelSources();
    builtinModelSourcesRegistered = true;
  }
  const { listModelSources, getSharedModelServicesFactory } = await loadModelSourceRegistrar();
  ...
  if (makeShared === undefined) return [];
  const shared = makeShared(agentDir);
  for (const { registrar, spec } of resolved) {
    registrar.register(shared.modelRegistry, spec, modelSourceLog);
  }
  // The exact one-liner pi SDK's rpc-mode uses for `get_available_models`.
  return shared.modelRegistry.getAvailable();
}

/** Deterministic rich question fixture shared by ext-askq request/response. */
const STUB_ASK_QUESTION_GROUP = {
  questions: [
    {
      header: "Release path",
      ...
      options: [
        {
          label: "Canary",
          description: "Roll out gradually with early health checks.",
        },
        {
          label: "Direct",
          description: "Release to all users in one step.",
        },
      ],
      allowOther: true,
    },
...
async function initPersistence() {
  ...
  try {
    const { sessionStoreConfigFromEnv } = await loadServerStoreApi();
...
/** Which extension-UI step the turn is paused on: null | "askq" | "select" | "input" | "editor" | "confirm". */
let pendingUi = null;
...
async function handlePrompt(cmd) {
  ...
  // Sentinel `402-retry`(402-UI ③ e2e):模拟上游 402 insufficient_balance 触发自动重试、
  // 重试耗尽后空手收尾——驱动 translate 层 auto_retry_start/end → data-pi-auto-retry 状态条
  // (402-UI ①)以及 agent_end 无文本兜底合成 error(402-UI ②)。errorMessage 文案模拟真实
  // pi-coding-agent formatProviderError() 拼出的 "<status>: <body>" 形态(OpenAI 风格 402 body)。
  if (msg.toLowerCase().includes("402-retry")) {
    const errorMessage =
      '402: {"error":{"code":"insufficient_balance","message":"Your account balance is insufficient"}}';
    await persistMessage(buildUserMessage(msg));
    write({ type: "agent_start" });
    write({ type: "turn_start" });
    write({ type: "response", id: cmd.id, command: "prompt", success: true });
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      write({ type: "auto_retry_start", attempt, maxAttempts, delayMs: 200, errorMessage });
      await new Promise((r) => setTimeout(r, 30));
      write({ type: "auto_retry_end", success: false, attempt, finalError: errorMessage });
    }
    // 重试耗尽:agent_end 无 assistant 文本、willRetry:false → translate 层空手收尾兜底。
    write({ type: "agent_end", messages: [], willRetry: false });
    return;
  }
  ...
  // `ext-askq`:用 protocol 共享 codec 发富问题组,应答后单步结束;
  // 否则维持原单步 confirm。各分支都不破坏现有非 sentinel 行为。
  awaitingUiResponse = true;
  const sentinel = typeof cmd.message === "string" ? cmd.message.toLowerCase() : "";
  if (sentinel.includes("ext-askq")) {
    const { encodeAskRequest } = await loadAskQuestionCodec();
    const request = encodeAskRequest(STUB_ASK_QUESTION_GROUP);
    pendingUi = "askq";
    write({
      type: "extension_ui_request",
      id: "askq-1",
  ...
  } else if (sentinel.includes("ext-select")) {
    pendingUi = "select";
    write({
      type: "extension_ui_request",
      id: "sel-1",
...
/** 单调递增的 confirm 请求序号:每轮 confirm 用唯一 id。 */
let confirmSeq = 0;

/** 发出 confirm 扩展 UI 请求并把暂停步标记为 confirm。 */
function writeConfirm() {
  pendingUi = "confirm";
  // 每次发新 confirm 都用唯一 id(镜像真实 agent 的 per-request 关联 id)。前端 PiInteraction
  // 以「id 已在留痕中」为去重判据抑制重复渲染(pi-interaction.tsx active 计算):若跨轮复用同一
  // 固定 id,第二轮的 confirm 会被误判为已应答而不再激活(单会话内连续两轮的回归)。
  confirmSeq += 1;
  write({
    type: "extension_ui_request",
    id: `ext-${confirmSeq}`,
...
async function handle(cmd) {
  switch (cmd.type) {
    ...
    case "extension_ui_response":
      ...
      if (pendingUi === "askq") {
        const { decodeAskAnswers } = await loadAskQuestionCodec();
        const decoded = decodeAskAnswers(cmd.value ?? "", STUB_ASK_QUESTION_GROUP);
        const summary =
          decoded.kind === "rich"
            ? decoded.answers.answers
                .map((answer) => {
                  const selected = answer.selected.join(", ") || "(none)";
                  return `${answer.header}: ${selected}${answer.other ? `; Other: ${answer.other}` : ""}`;
                })
                .join(" | ")
            : `degraded: ${decoded.rawValue}`;
        emitText([` AskUserQuestion answer: ${summary}`], `AskUserQuestion answer: ${summary}`);
        await finishTurn();
      } else if (pendingUi === "select") {
        writeConfirm();
      } else {
        await finishTurn();
      }
      ...
      if (
        ...
        if (p.domain === "canvas") {
          ...
          if (A_TIER.includes(p.action)) {
            ...
            // ★ B 档 register 的 att_ 是**真实上传**产生的(浏览器 POST /sessions/:id/attachments),
            // 所以这里必须签出**真实 HTTP 分发 URL**,而不是像 A 档那样现造 data: URI。
            //
            // 此前一律用 data: URI,导致 pane 里的舞台主图(`crossOrigin="anonymous"`)永远走不到
            // 跨源路径 —— data: 不受 CORS 约束。于是「B 档旋转」e2e 一直是绿的,真机却裂图
            // (分发端点当时没回 Access-Control-Allow-Origin)。**假绿的根在夹具,不在断言。**
            //
            // 签名由 createChildAttachmentStore(同 DIR + 同 secret)产出,主进程可校验 —— 这正是
            // 该工厂的用途。env 未下发附件配置时降级回 data: URI,不让 stub 崩。
            const realUrl = await presignStubAttachment(id);
            const fresh = stubCanvasAsset(id, {
              ...(realUrl !== undefined ? { displayUrl: realUrl } : {}),
              origin: "tool-output",
    ...
    case "get_available_models": {
      // Union of the deterministic baseline (unchanged, so PI_WEB_DEFAULT_MODEL=
      // stub-model and existing frame behavior keep working) with whatever the
      // REAL registerBuiltinModelSources() pipeline resolves from env/providers.json
      // (spec multi-gateway-providers, task 8.2, Req 11.3) — see buildCatalogModels().
      let catalogModels = [];
      try {
        catalogModels = await buildCatalogModels();
      } catch (err) {
        // Fail-soft: a broken catalog assembly must not take down the whole
        // session's model list (baseline models still ack).
        process.stderr.write(
          `model-catalog-stub-agent: buildCatalogModels failed: ${String(err)}\n`,
        );
      }
      write({
        type: "response",
        ...
        success: true,
        data: { models: [...AVAILABLE_MODELS, ...catalogModels] },
      });
      break;
    }
    case "get_commands":
      // 装配帧不再搭车 get_commands(spec runner-ready-frame,4.2 去搭车化):
      // 探针机制已删除,get_commands 不再必然到来 —— 声明帧改为启动期主动发射
      // (见 emitAssemblyDeclarations),与真实 runner 的装配期发射时机对齐。
...
/**
 * 装配期声明帧 + 就绪通告(spec runner-ready-frame,4.2 去搭车化)。
 *
 * 原先 slash_completions / agent_routes 两帧搭车 get_commands(readiness 探针)发出 ——
 * 探针机制删除后 get_commands 不再必然到来,改为启动期主动发射一次(与真实 runner 的
 * `emitSlashCompletions` / 装配期声明帧时机对齐)。随后发 `runner_ready`:custom 形态的
 * PiSession 收帧即 ready;cli 形态不识别该帧但会被放行忽略,靠单发 get_commands 就绪 ——
 * stub 无条件发帧即两态兼容。
 */
function emitAssemblyDeclarations() {
  ...
  // agent-declared-routes:装配期声明帧(PiSession active-gate 前缓存,重复发射幂等)。
  write({ type: "agent_routes", routes: DEMO_AGENT_ROUTES });
  // 就绪通告:声明帧之后发(声明先于就绪,与真实 runner 的装配→ready 顺序一致)。
...
let chain = initPersistence().then(() => {
  // 持久化就绪后即宣告可服务(此前收到的命令已在 chain 上排队,不丢失)。
  emitAssemblyDeclarations();
});
