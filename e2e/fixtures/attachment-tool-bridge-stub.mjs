 ...
 * (so the whole rpc-channel → session-engine → SSE → @pi-web/react → <PiChat>
 * chain runs unchanged), and persists to the same SessionEntryStore for resume.
 * Loaded via `--import jiti/register` (cwd = @pi-web/server pkg dir) so it can
 * import the TS-source `@pi-web/server` attachment-bridge.
 */
...
// @pi-web/server source are cwd-independent (the stub runs with cwd = the
// @pi-web/server package dir, so a bare relative path would be ambiguous).
...
async function getBridge() {
  ...
  // The example-tool factory is intentionally NOT in the @pi-web/server barrel
