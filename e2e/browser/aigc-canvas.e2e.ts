 ...
 *
 *  ① 闭环(生成经对话流,A 方案):选 `aigc-canvas-agent` → launcherRail 入口开画廊 → 种子图入 9 宫格 →
 *     点格子展开工作台 → 输入指令 → 点「生成」→ 组装 image_edit 指令经 `/messages` 发用户消息 →
 *     LLM(stub)调工具生图 → 轮末 auto-sync 收编 → 新图进画廊。断言**有** `/messages` 且用户
 *     气泡含 image_edit(操作回流对话历史;旁路 surface 命令仅剩 sync/register 等非生成操作)。
...
test("canvas: 闭环(launcherRail 入口 → 画廊 → 格子展开 → 生成经对话流 → 新图进画廊),操作回流对话历史", async ({
  page,
}) => {
  ...
  // 生成走对话流(A 方案):点「生成」→ 组装 image_edit 指令经 /messages 发用户消息 →
  // LLM(stub)调工具生图 → 轮末 auto-sync 收编 → 新图入画廊(回到画廊后 2 格)。
  // 操作因此回流对话历史:用户消息气泡可见(含 image_edit 指令)。
  ...
  await expect(page.locator("[data-canvas-cell]")).toHaveCount(2, { timeout: 15_000 });
});
