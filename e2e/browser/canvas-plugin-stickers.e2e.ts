 ...
 *
 *  ① 贴纸闭环:选 `canvas-plugin-stickers` → 画廊 pane 开箱即在 → 种子图入格 → 点格子
...
async function openWorkbench(page: import("@playwright/test").Page): Promise<import("@playwright/test").FrameLocator> {
  await selectSource(page, STICKERS_SOURCE);
  // ★ 迁 pane 后画廊与插件都在 iframe 内;返回 FrameLocator 供各用例定位。
  ...
  // ★ launcherRail 随迁移撤掉(store 不跨 realm,留着是死按钮);pane 由 initialPaneIds
  // 开箱即在。原断言守的「面板贡献仍在」由下面「画廊在 pane 内挂载」直接承担。
...
test("stickers: 贴纸闭环(工具轨命名空间锚 → 舞台置层 → Inspector 调尺寸 → 拍平进位图)", async ({
  ...
  const stickerTool = frame.locator(`[data-canvas-tool="${STICKER_TOOL_ANCHOR}"]`);
  ...
  const stage = frame.locator("[data-canvas-stage]");
  ...
  const pluginLayer = frame.locator("[data-canvas-plugin-layer]");
  await expect(pluginLayer).toHaveCount(1);
  const emoji = frame.locator("[data-sticker-emoji]");
  await expect(emoji).toBeVisible();

  // 选中的插件图层 → Inspector 浮层出现(data-canvas-inspector),含尺寸滑杆。
  const inspector = frame.locator("[data-canvas-inspector]");
  ...
  const flatten = frame.locator("[data-canvas-layer-flatten]");
  await expect(flatten).toBeEnabled();
  await flatten.click();
  await expect(frame.locator("[data-canvas-plugin-layer]")).toHaveCount(0);
});
...
test("stickers: 风格迁移经 command 通道回流画廊(capability 白名单 + 按胜者 via 分道)", async ({
  ...
  await frame.locator(`[data-canvas-tool="${STICKER_TOOL_ANCHOR}"]`).click();
  await frame.locator("[data-canvas-stage]").click({ position: { x: 180, y: 140 } });
  await expect(frame.locator("[data-canvas-plugin-layer]")).toHaveCount(1);
  await frame.locator("[data-canvas-layer-flatten]").click();
  await expect(frame.locator("[data-canvas-plugin-layer]")).toHaveCount(0);

  // @引用:拍平产物成为 refCandidates → 引用其为参考图(referenceIds.length===1)。
  const refTrigger = frame.locator("[data-canvas-ref-trigger]");
  await expect(refTrigger).toBeEnabled();
  await refTrigger.click();
  await frame.locator("[data-canvas-ref-option]").first().click();
  await expect(frame.locator("[data-canvas-ref-chip]")).toHaveCount(1);

  // prompt "style:…" → styleTransferAction 胜出:按钮标签/锚点=插件 label/去前缀 id。
  await frame.locator("[data-canvas-prompt]").fill("style:油画");
  const generate = frame.locator("[data-canvas-generate]");
  ...
  const versionsBefore = await frame.locator("[data-canvas-version-item]").count();
  await generate.click();
  await expect
    .poll(async () => frame.locator("[data-canvas-version-item]").count(), { timeout: 15_000 })
