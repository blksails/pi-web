...
/**
 * 「沙箱项目表单可保存」用例会真实写入被服务项目(=仓库根)的 `.pi/sandbox.json`
 * (`{"enabled":true}`)。若不清理,**首轮通过后残留即成毒药**:次轮加载时开关已是勾选态,
 * `check()` 不产生 dirty,保存钮永远禁用 → 用例必挂(2026-07-10 全量回归实证,曾被误判
 * 为特性回归)。故保存用例前后都删该文件,使其自带幂等。
 */
// __dirname 而非 import.meta:playwright 把 e2e TS 按 CJS 转译,import.meta 会直接 SyntaxError。
const PROJECT_SANDBOX_JSON = join(
  resolve(__dirname, "..", ".."),
  ".pi",
  "sandbox.json",
);
const removeProjectSandboxJson = (): void => {
  rmSync(PROJECT_SANDBOX_JSON, { force: true });
};
...
test("settings: 沙箱项目表单可保存(写所服务项目的 .pi/sandbox.json)", async ({ page }) => {
  // 自带幂等:删除历史残留,使「启用沙箱」恒从未勾选态出发(见文件头注释)。
  removeProjectSandboxJson();
