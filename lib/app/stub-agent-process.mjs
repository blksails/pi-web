...
async function handlePrompt(cmd) {
  // Sentinel `/review`:模拟**纯扩展命令**(只经 ctx.ui 反馈、不触发对话轮)。真实模式由 runner 的
  // command-marker seam 在 prompt 后判定纯命令并写 piweb.command 标记;stub 取代 runner,故此处镜像:
  // 不发 turn、不持久 user message,仅 appendEntry piweb.command(text)。不发 agent_start → server 的
  // R11 命令-turn watcher 在窗口后合成 finish 解冻输入框。冷恢复时 server 经 loadCommandMarkers 合并,
  // 使该 /review 气泡仍在(spec plugin-system-unification R13)。
  const msg = typeof cmd.message === "string" ? cmd.message : "";
  if (msg.startsWith("/review")) {
    // "piweb.command":与 server PIWEB_COMMAND_CUSTOM_TYPE 一致(stub 沿用硬编码字面量,同 piweb.session)。
    await appendEntry({
      type: "custom",
      customType: "piweb.command",
      data: { text: msg },
    });
