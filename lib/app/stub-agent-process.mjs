...
function stubCanvasAsset(id, over = {}) {
  return {
    attachmentId: id,
    // stub 展示 URL(1x1 透明 PNG data URI);真实态走 att_ 签名 URL。
    displayUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+M8AAAMBAQBN4h0FAAAAAElFTkSuQmCC",
    mimeType: "image/png",
    name: `${id}.png`,
      ...
      if (
        ...
        if (p.domain === "canvas") {
          ...
          if (A_TIER.includes(p.action)) {
            ...
            const next = { assets: [fresh, ...gallery.assets] };
            ...
            const next = { assets: [fresh, ...gallery.assets.filter((a) => a.attachmentId !== id)] };
            stubState.set(key, next);
            emitState(key, next);
            data = { ids: [id] };
          } else if (p.action === "sync") {
            emitState(key, gallery);
            data = { count: gallery.assets.length };
          } else if (p.action === "delete") {
            const id = String(p.args?.attachmentId ?? "");
            const next = { assets: gallery.assets.filter((a) => a.attachmentId !== id) };
            stubState.set(key, next);
            emitState(key, next);
            data = { deleted: id };
          } else {
