 ...
 * from @blksails/server) plus a `createChannel` seam, injects config defaults,
...
import { makeProjectTrustPolicy } from "@blksails/server/trust";
...
import {
  ...
  excludeProviders,
} from "@blksails/server/model-options";
...
function stubAgentPath(): string {
  ...
  if (override !== undefined && override !== "") {
    ...
    // (= the `@blksails/server` package dir). `path.resolve` passes an absolute
 ...
 *
 * `--import jiti/register` lets the stub `.mjs` import the TS-source `@blksails/server`
...
function stubSpawnSpec(
  ...
  // Run with cwd = @blksails/server package dir so `--import jiti/register`
