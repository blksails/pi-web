 ...
 *
 * `content` scans the app's own files AND the `@blksails/ui` source, because the
 ...
 * `@blksails/ui` preset (`packages/ui/tailwind-preset.ts`), so downstream apps can
 * adopt the same tokens with one `presets:` entry. Colors map to the shadcn CSS
 * variables provided by `@blksails/ui/styles.css`, so no colors are hardcoded.
 */
const config: Config = {
  ...
  content: [
    "./app/**/*.{ts,tsx}",
    "./components/**/*.{ts,tsx}",
    "./node_modules/@blksails/ui/src/**/*.{ts,tsx}",
