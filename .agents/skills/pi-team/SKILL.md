---
name: pi-team
description: 在 Codex 里把 pi-web 的多个开发任务编排成一支「agent 团队」——你(Codex)就是主控台,每个 Ghostty tab 是一个在独立 git worktree 里执行的 Codex agent。当用户想「并行跑多个任务/开一队 agents/用 tab 管理多个任务/主控台编排/把这些任务分给多个 agent 去做」时使用。底层用 ghostty-ctl(gtmux)管理 tab + scripts/pi-team/pi-team 管队列与 worktree。仅限 macOS + Ghostty。
---

# pi-team —— Codex 作主控台,多 tab 多 agent 编排

## 角色模型
- **主控台 = 你(Codex 本会话)**。不要起独立的后台守护进程;调度循环由你在对话里驱动。
- **每个 Ghostty tab = 一个在执行的 Codex agent**,跑在自己的 git worktree(分支 `pi-team/<id>`)里,互不冲突。
...
2. **派发**:`P start --max N`(默认并发 2–3,按用户意愿)。每个任务会:建 worktree → `gtmux claim` 一个 tab → 在 worktree 里启动交互式 `Codex` 并预载 prompt、自动提交首条。
...
## 约定与注意
- **并发名额靠 `done` 释放**:交互式 agent 无法自动判完成。你 peek 判断某任务完成或用户说完成后,`P done <id>` 再 `P start` 补位。(若用户要全自动,可改用 headless 执行体:`PI_TEAM_CMD='Codex -p --dangerously-skip-permissions'`,进程退出即完成,便于自动检测。)
