...
- `video_workflow` / `video_analyze` 已接入 Agent Surface；失败工作流不提交本地暂态修改，暂停返回 checkpoint。
...
- `pnpm -C examples/agic-video-agent test`：19/19 通过，含真实八镜头 MP4 解码 POC。
...
- 把 `VideoProject` attachment 解析接入 Renderer Adapter，完成图片信息流与真实 MP4 attachment 回流。
- 再做至少 3 类转场与多层 VFX 的引擎矩阵 POC；当前 FFmpeg 旧版能力不足，须保留降级标记。
