# 12 · Web UI 扩展（agent-web-extension）
...
> **两套「表面」勿混淆**：本章讲的是 5-tier **挂载机制**（把组件填进宿主让出的插槽 / 注册渲染器 / iframe）。其中 Tier4 的 `artifactSurface` 是 iframe 隔离表面。与之**正交**的还有一套 **Surface 权威表面**通信约定（`createSurface`/`useSurface`，命令上行 + 领域状态下行的 CQRS 单写者，端到端驱动 Canvas），它不是新的 tier，而是与挂载机制平行的通信平面，详见 [04 Surface 权威表面栈](./04-surface-stack.md)。
...
|------|------|------|-------------|
| 1 | **区域插槽** | 填入 21 个具名 slot（background、header、panelRight、promptToolbar、logs 等） | 是 |
...
1. **试现成示例（最快）** — 直接体验仓库内 `examples/webext-layout-agent`，无需自己写：
   ```bash
   pnpm dev   # dev-all.mjs：vite 前端 http://localhost:5173（/api 自动代理到 API 的 3000）
   ```
   浏览器打开 **http://localhost:5173**（HMR + SPA；3000 是 API-only 宿主，dev 下不提供前端页面）。在 agent source 输入框（`data-agent-source-input`，占位文案 `./examples/hello-agent or https://github.com/org/repo`）填 `./examples/webext-layout-agent` 提交。
...
6. **没生效？** — 多为签名/版本/门控问题，对照 [23 故障排查 FAQ](./23-troubleshooting-faq.md) 第 3 节「Web Extension / UI 问题」，或本章末尾「常见问题」。
...
```bash
# 在 agent source 根目录执行（@blksails/pi-web-kit 的 bin 名即 pi-web → build/cli.ts）
pnpm pi-web build \
  --id my-agent-ext \
  --api "^0.1.0" \
  --dir .pi/web \
  --out .pi/web/dist
  # 可选：--sign <ed25519PrivateKeyBase64Pkcs8> 为 manifest 写入 Ed25519 签名
```
...
> **对应可跑示例**：本 Tier 有三个落地示例——`examples/webext-layout-agent`（`panelRight` 领域检视面板 + `headerCenter`，见 `examples/webext-layout-agent/.pi/web/web.config.tsx:1`）、`examples/webext-slots-agent`（18 个区域插槽 fixture，覆盖原始 19 个保留槽中除 `logs` 外的全集，每槽一个带 `data-testid` 的可见组件，见 `examples/webext-slots-agent/.pi/web/web.config.tsx:1`；`launcherRail`/`promptToolbar` 为后续新增，不在此 fixture 内）、`examples/webext-background-agent`（`background` 区域插槽自定义动画极光背景，类名自命名空间，见 `examples/webext-background-agent/.pi/web/web.config.tsx:1`）。

### 21 个协议保留插槽

`SlotKeySchema` 是闭集枚举，当前 **21** 个（`packages/protocol/src/web-ext/descriptor.ts:28-53`）：前 19 个为原始保留槽，`launcherRail`、`promptToolbar` 为后续新增。
...
| `promptToolbar` | 输入框工具排内联槽，排在内核控件（附件/模型/语音/联网）之后、发送键之前；供 source 挂领域快捷设置控件，宿主不认内容语义 | — |

**插槽语义**：扩展内容以追加（additive）方式挂载，不替换内核表面。宿主未声明对应插槽时忽略、不报错（Req 2.3）。`launcherRail` 的渲染条件是「全局门控 `NEXT_PUBLIC_PI_WEB_LAUNCHER_RAIL=1` 开启 **或** 当前 source 声明了 `launcherRail` 贡献」二者其一——**source 声明贡献即视为意图，免全局门控**（`components/chat-app.tsx:787-790`，Canvas 即走此路）；两者都未命中时导航区不为该槽占位。

`promptToolbar` 是 AIGC 快捷设置控件的挂载点：装配期 `aigcExtension` 把 `aigc.models`/`modelLabels`/`sizes`/`enablePromptOptimization` 下发到会话共享状态，驱动提示词栏动态渲染模型/尺寸/视觉模型选择器（`packages/tool-kit/src/aigc/extension.ts:35-68`）。详见 [11 AIGC 与视觉工具](./11-aigc-and-vision-tools.md) 的「工具排快捷设置」。

### background 插槽的 isolate 陷阱

`background` 渲染在 `absolute inset-0 -z-10`。宿主用 Tailwind `isolate` 为聊天主列建立独立 stacking context（`packages/ui/src/chat/pi-chat.tsx:1645`），使负 z-index 被限定于此列之内——**而非逃逸到根上下文被 app-shell 不透明壳底遮挡**。

```tsx
// pi-chat.tsx:1645（宿主实现细节，扩展作者无需改动）
...
> **对应可跑示例**：`examples/webext-renderer-agent`——同时注册 `echo` 工具卡渲染器（`EchoToolRenderer`）与 `data-metric` data-part 渲染器（`MetricRenderer`），并在 agent `index.ts` 注册配套的 `echo` customTool 驱动触发（见 `examples/webext-renderer-agent/.pi/web/web.config.tsx:1`）。
...
> **对应可跑示例**：`examples/webext-contrib-agent`——slash 命令、@mention、autocomplete、inlineComplete、keybindings 贡献点全集，全部经 `ui-rpc` 总线回 agent 处理（见 `examples/webext-contrib-agent/.pi/web/web.config.tsx:1`）。
...
#### keybindings 的运行时语义：填充而非执行

`keybindings` 声明 `combo → commandId`。宿主在**会话作用域**注册一个 `document` 级 `keydown` 监听，按 `mod`/`shift`/`alt` 修饰键 + 主键匹配 combo（`mod` 同时匹配 `metaKey || ctrlKey`）。**命中后并不直接执行命令，而是把 `/${commandId} ` 填入输入框**（`e.preventDefault()` 后 `setInput`），由用户确认再发送（`packages/ui/src/chat/pi-chat.tsx:1196-1228`，DOM 上 `data-pi-keybindings`）。这是刻意的：避免快捷键静默触发副作用，把最终执行权留给用户。切勿把它当作「按键即执行」。
...
- 开启条件是 `needsIdleControl && !isBusy`：`needsIdleControl` 由 `hasContributions`、`hasArtifactRpc`、`panelRight` surface 槽（`hasSurfacePanel`）以及就绪握手未就绪期任一命中；prompt 流传输期间关闭（由 per-prompt 流处理 control 帧），**避免并发冲突**（`packages/ui/src/chat/pi-chat.tsx:696-753`）。

```typescript
// pi-chat.tsx:696-753（宿主逻辑，简化）
...
const needsIdleControl =
  hasContributions || hasArtifactRpc || hasSurfacePanel; /* || 就绪握手未就绪期 */
React.useEffect(() => {
...
> **对应可跑示例**：`examples/webext-artifact-agent`——声明 `artifact.entry`，宿主用 `sandbox="allow-scripts"` iframe 加载 `artifact.html`，经 postMessage 完成 resize / rpc 双向通信（见 `examples/webext-artifact-agent/.pi/web/web.config.tsx:1`）。运行需设置 `NEXT_PUBLIC_PI_EXTENSION_BASE_URL`，详见下文「门控」小节。
...
3. 双向通信经 postMessage，消息结构由 `@blksails/pi-web-protocol` 的 `ArtifactMessage` 类型约束：
...
`ArtifactSurface` 的 `src` 由 `extensionBaseUrl + artifact.entry` 拼接。**若 `extensionBaseUrl` 为空，`ArtifactSurface` 不会挂载**——这是正确的门控行为，不是 bug（`components/chat-app.tsx:967-968`，`extensionBaseUrl().length > 0` 才注入）。

> **变量名保留，语义已反转**：`NEXT_PUBLIC_*` 前缀是历史命名，但**不再是构建期内联**。现在由服务端 `GET /api/bootstrap` **每次请求读进程 env** 后下发（`server/bootstrap.ts:105` 取 `env.NEXT_PUBLIC_PI_EXTENSION_BASE_URL ?? ""`，装进 `features.extensionBaseUrl`），前端经 `BootstrapGate` → `setRuntimeFeatures` → chat-app 的 `extensionBaseUrl()` 消费（`src/bootstrap.tsx:140`）。因此它是**运行时**开关：在**启动 API server 的进程**上设好该 env 即生效，改值后重启该进程（无需重新构建）。

```bash
# 启动前设进 API server 进程的环境（dev 下由 dev-all 拉起的 API 进程读取）
# dev：与 SPA 同源时用应用地址
export NEXT_PUBLIC_PI_EXTENSION_BASE_URL=http://localhost:5173
# 生产：指向独立托管 artifact 资源的源
# export NEXT_PUBLIC_PI_EXTENSION_BASE_URL=https://ext.example.com
```

仍不出现 iframe 时，按 [23 故障排查 FAQ](./23-troubleshooting-faq.md) 第 3.1 节核对。

---

## Tier 5：纯声明配置（config）

> **对应可跑示例**：`examples/webext-declarative-agent`——零代码、零 bundle，只有一个手写的 `.pi/web/manifest.json`（紫色 `theme` token + `layout: "wide"` + `empty` 空态文案与 starters），宿主直接合成描述符（见 `examples/webext-declarative-agent/.pi/web/manifest.json:1`）。
...
选定 agent source → 宿主读取 .pi/web/dist/manifest.json
  ...
  │            ↓ 通过
  │         注入 import map（react/react-dom/@blksails/pi-web-kit → 宿主单例 URL）
...
import map 已随 SPA 入口 `index.html` 的 `<head>` **静态内置**（内联，生产 CSP 经 sha256 hash 放行），把裸 `import "react"`/`"react/jsx-runtime"`/`"react-dom"`/`"@blksails/pi-web-kit"` 解析到 `/api/webext/singletons/<name>` 宿主单例端点，避免 hook 冲突。其内容须与 `lib/app/webext-singletons.ts` 的 `WEBEXT_IMPORT_MAP` 逐字一致（漂移由 `test/webext-import-map.test.ts` 拦截）。

---

## webext 包安装与运行时加载（webext-package-install）

上节「加载流程」描述的是构建期已知源；**安装的 webext**（随 plugin 包落盘、构建期注册表未命中的源）走运行时车道:

```
/plugin install <source>（或已装源）→ 落盘（复用 pi install）
  │
  ▼  会话激活时
GET /api/webext/resolve?source=<source>
  ├─ 服务端 locateDist → 读 <installedPath>/.pi/web/dist/manifest.json
  ├─ 服务端验签（trust-service，见下）→ 产出 VettedManifest（去 signature、保 integrity）
  └─ 返回 { found, manifest(已背书), baseUrl }
  │
  ▼  客户端（useRuntimeWebext）
loadExtension：纯声明→直接 apply config；代码→fetch .mjs → 浏览器 SRI → 动态 import → applyExtension
```

- **落盘复用 pi install**：webext 产物 `.pi/web/dist/` 随其 npm/git 包被装到 `~/.pi/agent/npm/node_modules/<pkg>/`；不重做安装器。
- **静态托管**：`GET /api/webext/dist/<base64url(distDir)>/<file>` 只读托管产物（realpath 前缀校验防目录穿越）。

### 信任模型：签名服务端验 / SRI 浏览器验

webext 代码在**浏览器同源**执行（共享 React 单例），等于可执行任意代码，故:

| 校验 | 算法 | 位置 | 说明 |
|---|---|---|---|
| 完整性 SRI | sha384 | **浏览器** | 字节比对 manifest.integrity；无需机密 |
| 发布者签名 | **Ed25519** | **服务端** | 公钥验签；机密/材料不下发浏览器 |

服务端验签通过后产出去签名的 `VettedManifest`（含 integrity），浏览器仅凭它做 SRI（gate 的 `signaturePreVerified` 跳过签名分支、仍验 SRI）。构建侧 `pi-web build --sign <ed25519PrivateKeyBase64Pkcs8>` 签名。

### 可信发布者白名单与中心列表

- 白名单（`PI_WEB_EXT_WHITELIST`，Ed25519 公钥）为**服务端信任根**，由部署方掌握，终端用户/扩展不可改。
- 可选**中心可信发布者列表**（`PI_WEB_EXT_TRUSTED_LIST_URL` + 出厂钉死根公钥 `PI_WEB_EXT_ROOT_PUBKEY`）：下载列表须经根公钥验签方采信；拉取失败回退缓存/出厂快照，**绝不 fail-open**；运营者本地决策（吊销/追加/固定版本/停用）优先于中心列表。
- `PI_WEB_EXT_REQUIRE_SIGNATURE`（默认 true）：生产强制签名；`false` 仅本地开发免签（带不安全提示）。

### CSP 与单例

- 动态加载用 `import(/* webpackIgnore: true */ url)`（避打包器改写且**无需 `unsafe-eval`**，可在禁 eval 的生产 CSP 下工作）。
- 单例 ESM 端点 `GET /api/webext/singletons/<react|react-jsx-runtime|react-dom|webkit>` 从 `window.__PI_WEBEXT_SINGLETONS__`（宿主桥接注入）re-export 宿主同一实例；import map 把裸 specifier 映射到这些端点。

### 装后双路生效

含 webext 的包安装后:① pi 资源经 `SessionReloader`（runner reload）生效；② webext 经客户端重触发加载路径生效（`useRuntimeWebext` 的 reloadNonce）。两路并行，避免「装了但 UI 没变」。

---

## 示例索引（examples/）

五层模型的每一层都有可直接跑通的示例。下表按 **Tier → 示例**速查，宿主加载方式见上文「端到端：从零跑通一个扩展」：

| Tier | 能力 | 对应示例 |
|------|------|----------|
| Tier1 | 区域插槽 | `examples/webext-layout-agent`、`examples/webext-slots-agent`、`examples/webext-background-agent` |
| Tier2 | 自定义渲染器 | `examples/webext-renderer-agent` |
| Tier3 | 贡献点 + RPC | `examples/webext-contrib-agent` |
| Tier4 | Artifact iframe | `examples/webext-artifact-agent` |
| Tier5 | 纯声明配置 | `examples/webext-declarative-agent` |
...
| `examples/webext-slots-agent/` | Tier1+5 | 18 个区域插槽 fixture（原始 19 槽中除 `logs` 外的全集）+ 空态声明式配置验收 |
...
> 全部示例的总索引（含非 webext 示例）见 [`examples/README.md`](https://github.com/blksails/pi-web/blob/main/examples/README.md)。
...
A：检查启动 API server 的进程是否设置了 `NEXT_PUBLIC_PI_EXTENSION_BASE_URL`（现由 `GET /api/bootstrap` 服务端读 env 后下发，非构建期内联）。为空时宿主不挂载 `ArtifactSurface`，这是正确门控，非 bug（`components/chat-app.tsx:967-968`）。改值后重启该进程。
...
A：`split` 仅声明布局意图，须同时在 `slots.panelRight` 提供实际组件；否则宿主不渲染 aside 容器，自动退化为居中版面（`pi-chat.tsx:1816`）。
...
- **Surface 权威表面**（与 5-tier 正交的 CQRS 通信平面，`createSurface`/`useSurface`，勿与 Tier4 `artifactSurface` 混淆）→ [04 Surface 权威表面栈](./04-surface-stack.md)
- 扩展与技能安装管理 → [10 扩展与 Skills](./10-extensions-and-skills.md)
- AIGC 图像/视觉工具与 `promptToolbar` 快捷设置（`aigc.models` 下发）→ [11 AIGC 与视觉工具](./11-aigc-and-vision-tools.md)
- 声明式 Config UI 与动态 widget → [13 Config UI](./13-config-ui.md)
- Canvas 工作台的提示词栏「解读」按钮（`vision-op` 把工作图+问题组装成 `tool:image_vision` 的 SurfaceOp 回流对话，跨 webext/Canvas 边界；模型取 `provider/modelId` 格式，勿与生成模型裸 id 混用）→ [16 Canvas 工作台](./16-canvas-workbench.md)
- 浏览器 e2e 隔离构建跑法 → [22 开发与测试](./22-development-and-testing.md)
- `POST /sessions/:id/ui-rpc`、webext 系列端点 → [24 HTTP API 参考](./24-http-api-reference.md)
