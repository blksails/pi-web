...
|------|------|------|-------------|
| 1 | **区域插槽** | 填入 19 个具名 slot（background、header、panelRight、logs 等） | 是 |
...
设置后需重启 dev（`NEXT_PUBLIC_*` 在构建/启动期注入，运行时改 `.env.local` 不热更）。仍不出现 iframe 时，按 [23 故障排查 FAQ](./23-troubleshooting-faq.md) 第 3.1 节核对。
...
| `examples/webext-slots-agent/` | Tier1+5 | 18 个区域插槽 fixture（协议 19 槽中除 `logs` 外的全集）+ 空态声明式配置验收 |
...
- AIGC 图像生成工具（与 artifact 表面结合使用）→ [11 AIGC 工具](./11-aigc-and-vision-tools.md)
- 浏览器 e2e 隔离构建跑法 → [22 开发与测试](./22-development-and-testing.md)
- `POST /sessions/:id/ui-rpc` 端点 → [24 HTTP API 参考](./24-http-api-reference.md)
