...
> 常见报错：若构建后页面报 webpack 500，多半是与运行中的 `next dev` 共享了 `.next`（见 [23 · 1.1](./23-troubleshooting-faq.md)）；若 Playwright 端口被占用或下载受限，先用上文外部服务器模式或退回 `pnpm e2e:node`。测试与工具链类问题汇总见 [23 · 4 测试与工具链问题](./23-troubleshooting-faq.md)。
...
- `packages/server`、`packages/protocol` 等子包边界 → [05 包结构](./05-packages.md)
- `SESSION_STORE`、`PI_WEB_ATTACHMENT_DIR` 等环境变量 → [06 配置](./06-configuration.md)
- `build:cli` standalone 构建与 `bin/pi-web.mjs` → [18 CLI](./18-cli.md)
- 生产构建与服务器启动 → [19 部署](./19-deployment.md)
- 测试环境日志配置 → [21 日志](./21-logging.md)
- 构建污染、e2e 端口冲突等问题排查 → [23 常见问题](./23-troubleshooting-faq.md)
