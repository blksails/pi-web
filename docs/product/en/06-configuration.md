...
| `PI_WEB_SOURCES_ROOT` | (unset, no scan) | Directory scan roots, `path.delimiter`-separated (`:` / `;`); relative paths resolve against `PI_WEB_DEFAULT_CWD` into absolute paths. Scans each root's first-level subdirectories: with an `index.[jt]s` entry → `custom`, otherwise → `cli` |
