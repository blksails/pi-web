...
> For more detailed error reproduction and troubleshooting steps (signed URL 401, subprocess `ctx.available === false`, upload 413, etc.) see [23 · Troubleshooting FAQ](./23-troubleshooting-faq.md).

---

## Next Steps / Related

- How AIGC image tools call the attachment system → [11 · AIGC Tools](./11-aigc-and-vision-tools.md)
- Trigger completion framework / `@`-mention attachments → this doc [§9](#9-trigger-completion-framework--mention-attachments); extension points also in [10 · Extensions and Skills](./10-extensions-and-skills.md)
- Full HTTP API endpoint list (including `/attachments`) → [24 · HTTP API Reference](./24-http-api-reference.md)
- Overall system architecture and process boundaries → [03 · Architecture](./03-architecture.md)
- Environment-variable configuration for deployment → [19 · Deployment](./19-deployment.md)
- Signed URL 401, `ctx.available === false`, and other troubleshooting → [23 · Troubleshooting FAQ](./23-troubleshooting-faq.md)
