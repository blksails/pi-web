...
3. **Isomorphic `@blksails/pi-web-logger` package** — zero runtime dependencies, no static Node module references, safe to import on both the Node and browser sides.

---

## The `@blksails/pi-web-logger` Package

**Package name**: `@blksails/pi-web-logger`  
**Location**: `packages/logger/`

### Core API

```typescript
import { createLogger, configureLogger, initConfigFromEnv } from "@blksails/pi-web-logger";
...
> **The file holds bare JSONL, with no sentinel**: the file sink writes `JSON.stringify(entry) + "\n"` per line (`packages/logger/src/file-sink.ts:146`), **without** the node-sink `LOG_SENTINEL` prefix. The output formats of the two sinks are deliberately different — stderr needs the sentinel so the main-process `parseLogLine` can sift log lines out of the RPC protocol stream, whereas the file is written to disk independently with no sifting required, so it is emitted as clean JSONL that `jq`/`tail` can consume directly. When troubleshooting file logs, do not grep for the `\x02PILOG\x03 ` prefix.
...
1. **runner bootstrap** — at runner startup, `initConfigFromEnv()` is called to initialize the Node-side logger config from `PI_WEB_LOG_*` env (`packages/server/src/runner/runner.ts:212`).
...
The panel is rendered directly by the kernel `PiChat` (`packages/ui/src/chat/pi-chat.tsx`), not as a standalone slot. `PiChat` decides whether and where to mount the panel based on three props — `showLogs` / `logsPanelVisible` (corresponding to `outputs.panelVisible`) / `logsPanelPosition` (corresponding to `outputs.panelPosition`). The `panelPosition` enum has four values (`packages/protocol/src/config/domains/logging.ts:103`), each rendering a container marked with `data-pi-logs-region`:

| `panelPosition` | Render location | Behavior |
|-----------------|----------|------|
| `bottom` (default) | Below the input dock (`pi-chat.tsx:1567`) | A horizontal panel stacked in the same column as the session usage bar |
| `top` | A horizontal log strip above the conversation / empty state (`pi-chat.tsx:1677`, `data-pi-logs-top`) | Centered at content width, scrolling within a bounded `max-h-56` height |
| `right` | A standalone block inside the right-hand `aside` (`pi-chat.tsx:1760`) | Coexists with `panelRight` / artifact in the same aside |
| `drawer` | A fixed bottom overlay (`pi-chat.tsx:1591`) | Toggled open/closed by the "Logs" button with `data-pi-logs-drawer-toggle` (`pi-chat.tsx:1581`); a `fixed` drawer at `max-h-[40vh]` |

Alongside the kernel `LogsPanel` at each position there coexists a webext `logs` slot (`ExtSlotRegion slot="logs"`, `pi-chat.tsx:1573 / 1596 / 1686 / 1765`): an extension's contributions to the `logs` slot render after the kernel panel with **append semantics** — the two coexist rather than replace each other. See `slots.logs` of the webext in `examples/*` for an example (wired in task 8.3).
...
Auto-scroll is implemented by `LogsPanel` itself (`packages/ui/src/logs/logs-panel.tsx`); its `handleScroll` determines bottom-pinning via `scrollTop + clientHeight >= scrollHeight - SCROLL_BOTTOM_THRESHOLD` (`logs-panel.tsx:233`) — independent of the `use-auto-scroll.ts` hook used generally by the conversation area, which the panel does not reuse:

- **Pinned follow** — when at the bottom, a new entry arriving sets `ul.scrollTop = ul.scrollHeight` to keep following, and `unreadCount` is reset to zero (`logs-panel.tsx:214`).
- **Pause on scroll-up** — scrolling up away from the bottom pauses the follow; during the pause new entries accumulate as the unread count by positive increments, while entry reductions caused by filtering (negative increments) are not counted (`logs-panel.tsx:219`).
- **Jump-to-unread button** — when paused and `unreadCount > 0`, a `data-pi-logs-jump-latest` button floats at the bottom right of the panel, with the text `↓ N new logs` (i18n `logs.unread`); clicking it returns to the bottom, resumes the follow, and resets unread to zero (`logs-panel.tsx:364`).

### Narrow-Column Adaptive Wrapping

`LogRow` uses an adaptive row layout (`logs-panel.tsx:86`): in a wide container the four columns — time / level / namespace / message — lay out on a single row; in a narrow container (such as the `right`-side column), the message column uses `flex-[1_1_12rem]` (12rem minimum width) to trigger the row container's `flex-wrap` (`logs-panel.tsx:143`), wrapping onto a full-width row and breaking by word (`break-words`), avoiding fixed columns squeezing the message into a character-by-character vertical layout.
...
- If there is still no output, see [23 · Troubleshooting FAQ](./23-troubleshooting-faq.md).

---

## Protocol: SSE Log Control Frame

Logs are pushed over the existing SSE control-frame channel. The top-level SSE frame is discriminated by `kind`; logs travel on a `kind: "control"` frame, with the inner `payload.control` being `"logs"` (plural), distinguished from other control events via the same `discriminatedUnion("control", …)` (`packages/protocol/src/transport/sse-frame.ts:21-54`). That discriminated union currently spans nine variants: `extension-ui` / `queue` / `stats` / `error` / `ui-rpc` / `logs` / `session-status` (the readiness-handshake sticky frame) / `state` (the downstream mirror of the state-injection bridge) / `session-state` (the session-authoritative snapshot sticky frame) — logs are just one branch, so when writing an SSE parser cover the full set rather than only the first four. A single frame can carry multiple `entries`:
...
- [05 · Packages](./05-packages.md) — `@blksails/pi-web-logger` is an isomorphic logging package that can be published and consumed independently (a leaf-root of the dependency tree)
- [06 · Configuration](./06-configuration.md) — `PI_WEB_*` env variables and the Settings UI framework
- [08 · Agent Development](./08-agent-development.md) — `AgentContext.logger` injection
- [10 · Extensions and Skills](./10-extensions-and-skills.md) — referencing the logging library directly in a pi extension
- [13 · Config UI](./13-config-ui.md) — the schema-driven settings surface of the `logging` config domain and the `logNamespaceToggles` widget
- [22 · Development and Testing](./22-development-and-testing.md) — unit tests and isolated-build E2E
- [23 · Troubleshooting FAQ](./23-troubleshooting-faq.md) — common log-related issues
- [24 · HTTP / SSE API Reference](./24-http-api-reference.md) — the full `control`-frame enumeration and the `GET /api/sessions/:id/logs` history endpoint
