...
- Session action endpoints (delete / rename / favorite) and the rest of the `/sessions/**` endpoints → [13 · HTTP/SSE API Reference](./13-http-api-reference.md), this chapter's [§5.1](#51-session-action-endpoints-delete--rename--favorite)
- Host `slots` and interface layout → [10 · Web UI Extension](./10-web-ui-extension.md)
- The session-management write gate `NEXT_PUBLIC_PI_WEB_SESSIONS_MANAGE` and environment-variables overview → [05 · Configuration Reference](./05-configuration.md), this chapter's [§9.5](#95-deployment-gate)
