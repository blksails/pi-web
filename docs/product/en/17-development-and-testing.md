...
> Common error: if the page reports a webpack 500 after a build, it is most likely sharing `.next` with a running `next dev` (see [18 · 1.1](./18-troubleshooting-faq.md)); if the Playwright port is occupied or downloads are restricted, use the external-server mode above first, or fall back to `pnpm e2e:node`. For a roundup of testing and toolchain issues, see [18 · 4 Testing and Toolchain Issues](./18-troubleshooting-faq.md).
...
- Sub-package boundaries such as `packages/server`, `packages/protocol` → [04 Packages](./04-packages.md)
- Environment variables such as `SESSION_STORE`, `PI_WEB_ATTACHMENT_DIR` → [05 Configuration](./05-configuration.md)
- The `build:cli` standalone build and `bin/pi-web.mjs` → [14 CLI](./14-cli.md)
- Production build and server startup → [15 Deployment](./15-deployment.md)
- Logging configuration for the test environment → [16 Logging](./16-logging.md)
- Troubleshooting build pollution, e2e port conflicts, and similar issues → [18 Troubleshooting FAQ](./18-troubleshooting-faq.md)
