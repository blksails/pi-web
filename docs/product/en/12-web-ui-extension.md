...
After setting it, restart dev (`NEXT_PUBLIC_*` is injected at build/startup time; editing `.env.local` at runtime does not hot-reload). If the iframe still does not appear, verify against [23 · Troubleshooting FAQ](./23-troubleshooting-faq.md) section 3.1.
...
- AIGC image generation tools (used together with the artifact surface) → [11 · AIGC Tools](./11-aigc-and-vision-tools.md)
- Running the browser e2e isolated build → [22 · Development & Testing](./22-development-and-testing.md)
- The `POST /sessions/:id/ui-rpc` endpoint → [24 · HTTP API Reference](./24-http-api-reference.md)
