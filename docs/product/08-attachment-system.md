...
> 类型契约 `AttachmentToolContext` / `AttachmentToolHandle` 由 `@pi-web/agent-kit` 暴露给 tool 作者（仅类型，无值导入）；构造函数 `createAttachmentToolContext()`（值）留在 `@pi-web/server`。
...
```ts
import { useAttachments } from "@pi-web/react";
...
```ts
import { uploadAttachment } from "@pi-web/react";
