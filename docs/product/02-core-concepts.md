...
3. **双模式判定** + 信任策略 → 生成一份 `spawnSpec`（子进程怎么起，由 `@pi-web/protocol` 定义类型）。
...
- `SpawnSpec`（子进程怎么起）由 `@pi-web/protocol` 拥有并导出，是单一事实来源；
...
前后端经 **SSE（Server-Sent Events）** 传流式数据，每帧携带 `protocolVersion`。`@pi-web/protocol` 是稳定契约，类型/schema 改动需语义化版本管理。详见 [13 HTTP/SSE API 参考](./13-http-api-reference.md)。
...
- 提到的 `@pi-web/protocol`、`@pi-web/server` 等包边界 → [04 分层包](./04-packages.md)
