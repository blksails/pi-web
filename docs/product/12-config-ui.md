...
import type { FieldProps } from "@pi-web/ui";

// 注：内置控件复用的 `FieldShell` 是包内相对模块，未从 @pi-web/ui 导出；
...
```ts
import { registerFieldRendererByKey } from "@pi-web/ui";
...
```ts
import { registerSettingsPanel, makeConfigDomainIO, zodValidator } from "@pi-web/react";
import { yourDomainFormSchema, yourDomainConfigSchema } from "@pi-web/protocol";
...
> **架构注**：schema 拉取走客户端是 `json-schema-config-form` spec 的明确交付决策（适配器在 `@pi-web/protocol` 前后端共享，浏览器直接 fetch 文件自带的 `$schema`），目的是避免服务端对任意 URL 的 SSRF 面、也避免把 schema 透传穿过 `useConfigDomain → SchemaForm` 管线。
>
...
- 配置文件与 env 变量详情（含 `PI_WEB_AGENT_DIR`）→ [05 配置参考](./05-configuration.md)
- Provider 与模型设置（`settings.json` 对应页）→ [06 Provider 与模型](./06-providers-and-models.md)
- 扩展配置域、沙箱注入 → [09 扩展与 Skills](./09-extensions-and-skills.md)
- HTTP API 端点完整列表（含 `GET·PUT /api/config/:domain`）→ [13 HTTP/SSE API 参考](./13-http-api-reference.md)
- widget 不显示 / 保存 422 / secret 被清空等问题 → [18 故障排查 / FAQ](./18-troubleshooting-faq.md)
