...
`createExtensionRoutes` 与 `createPiWebHandler` 均从 `@pi-web/server` 主入口导出（`packages/server/src/index.ts` 经 barrel `export *` 重导出 `extensions/index.js`；该包**未**暴露 `@pi-web/server/extensions` 子路径）：

```typescript
import { createExtensionRoutes, createPiWebHandler } from "@pi-web/server";
...
| 类型 | 格式示例 | 版本固定要求 |
|------|---------|------------|
| npm | `npm:@pi-web/my-ext@1.2.3` | 精确 semver `@x.y.z`（不允许 range/dist-tag） |
...
curl -X POST http://localhost:3000/extensions \
  ...
  -d '{"source": "npm:@pi-web/code-review@2.0.0"}'
...
PiSession → ControlStore.extensionUiQueue（FIFO，仅交互类请求入队）
  │  SSE control frame
  ▼
前端 useExtensionUI（@pi-web/react）
...
```
[ext-audit] {"actor":"alice","at":"2026-06-24T10:00:00.000Z","action":"install","source":"npm:@pi-web/code-review@2.0.0","outcome":"success"}
```
