...
> 可直接参考仓库 `examples/hello-agent/index.ts:1`（带自定义 `echo` 工具的最小可运行 agent）。详细用法见 [08 · Agent 开发](08-agent-development.md)。
...
> 插槽 key 取自 `SLOTS` 常量表（`SLOTS.panelRight` 等），亦可直接写字面量；可用插槽全集见 `packages/web-kit/src/slots.ts:8` 与 [12 · Web UI 扩展](12-web-ui-extension.md)。完整可运行范例见仓库 `examples/webext-slots-agent/.pi/web/web.config.tsx:1`。
...
- [06 · 配置参考](06-configuration.md) — `@blksails/pi-web-server/model-options` 与环境变量
- [08 · Agent 开发](08-agent-development.md) — `@blksails/pi-web-agent-kit` 与 `defineAgent()` 的详细用法
- [10 · 扩展与技能](10-extensions-and-skills.md) — `@blksails/pi-web-kit` 与 `defineWebExtension()`
- [11 · AIGC 工具](11-aigc-and-vision-tools.md) — `@blksails/pi-web-tool-kit/runtime` 的 `buildAigcTools`
- [24 · HTTP API 参考](24-http-api-reference.md) — `@blksails/pi-web-server` HTTP 模块的路由约定
