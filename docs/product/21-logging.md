...
3. **同构 `@blksails/pi-web-logger` 包** — 零运行时依赖，无静态 Node 模块引用，可在 Node 与浏览器两端安全导入。

---

## `@blksails/pi-web-logger` 包

**包名**：`@blksails/pi-web-logger`  
**位置**：`packages/logger/`
...
> **文件里是裸 JSONL，无 sentinel**：文件 sink 每行写的是 `JSON.stringify(entry) + "\n"`（`packages/logger/src/file-sink.ts:146`），**不带** node-sink 的 `LOG_SENTINEL` 前缀。两个 sink 的输出格式刻意不同——stderr 需要 sentinel 让主进程 `parseLogLine` 把日志行从 RPC 协议流里分拣出来，而文件是独立落盘、无需分拣，故写成可直接被 `jq`/`tail` 消费的干净 JSONL。排查文件日志时不要按 `\x02PILOG\x03 ` 前缀去 grep。
...
1. **runner bootstrap** — runner 启动时调用 `initConfigFromEnv()`，从 `PI_WEB_LOG_*` env 初始化 Node 侧 logger 配置（`packages/server/src/runner/runner.ts:212`）。
2. **PiSession 门控** — `handleStderr` 在会话激活时经注入的 `loggingConfigProvider` 加载 logging 配置；配置就绪前先缓冲 stderr chunk，就绪后回放并过滤。
3. **逐条过滤 + 入库** — `processStderrChunk` 对每条 `LogEntry` 依次套用 `gate.enabled` / `isLevelEnabled` / `isNamespaceEnabled`（来自 `@blksails/pi-web-logger`），通过者经 `LogRingBuffer.ingest` 分配 id 入会话环形缓冲，再合并为 `control:logs` 帧广播（`packages/server/src/session/pi-session.ts`）。
...
面板由内核 `PiChat` 直接渲染（`packages/ui/src/chat/pi-chat.tsx`），不是独立 slot。`PiChat` 据 `showLogs` / `logsPanelVisible`（对应 `outputs.panelVisible`）/ `logsPanelPosition`（对应 `outputs.panelPosition`）三项 props 决定是否及在何处挂载面板。`panelPosition` 枚举共四种取值（`packages/protocol/src/config/domains/logging.ts:103`），各自渲染一个带 `data-pi-logs-region` 标记的容器：

| `panelPosition` | 渲染位置 | 行为 |
|-----------------|----------|------|
| `bottom`（默认）| 输入 dock 下方（`pi-chat.tsx:1567`）| 与会话用量条同列堆叠的水平面板 |
| `top` | 对话/空态之上的横向日志条（`pi-chat.tsx:1677`，`data-pi-logs-top`）| 与内容同宽居中，`max-h-56` 有界高度内滚动 |
| `right` | 右侧 `aside` 内的独立区块（`pi-chat.tsx:1760`）| 与 `panelRight` / artifact 共存于同一 aside |
| `drawer` | 固定底部覆盖层（`pi-chat.tsx:1591`）| 由 `data-pi-logs-drawer-toggle`（`pi-chat.tsx:1581`）的「日志」按钮开合，`fixed` 抽屉 `max-h-[40vh]` |

每个位置的内核 `LogsPanel` 旁都并存一个 webext `logs` 插槽（`ExtSlotRegion slot="logs"`，`pi-chat.tsx:1573 / 1596 / 1686 / 1765`）：扩展对 `logs` slot 的贡献以**追加语义**渲染在内核面板之后，二者并存而非替换。示例见 `examples/*` 中 webext 的 `slots.logs`（task 8.3 接入）。
...
自动滚动由 `LogsPanel` 自身实现（`packages/ui/src/logs/logs-panel.tsx`），其 `handleScroll` 用 `scrollTop + clientHeight >= scrollHeight - SCROLL_BOTTOM_THRESHOLD` 判贴底（`logs-panel.tsx:233`）——与对话区通用的 `use-auto-scroll.ts` 钩子各自独立，面板未复用该钩子：

- **贴底跟随** — 处于底部时新条目到达即把 `ul.scrollTop = ul.scrollHeight` 续随，`unreadCount` 清零（`logs-panel.tsx:214`）。
- **向上暂停** — 用户上翻离底即暂停跟随；其间新条目按正向增量累计为未读数，过滤导致的条目减少（负增量）不计入（`logs-panel.tsx:219`）。
- **未读跳转按钮** — 暂停且 `unreadCount > 0` 时，面板右下角浮出 `data-pi-logs-jump-latest` 按钮，文案 `↓ N 新日志`（i18n `logs.unread`）；点击回到底部、恢复跟随并清零未读（`logs-panel.tsx:364`）。

### 窄列自适应换行

`LogRow` 采用自适应行布局（`logs-panel.tsx:86`）：宽容器下时间/级别/命名空间/消息四列单行排布；在窄容器（如 `right` 右侧栏）中，消息列以 `flex-[1_1_12rem]`（12rem 最小宽）触发行容器的 `flex-wrap`（`logs-panel.tsx:143`）换到整行全宽并按词换行（`break-words`），避免固定列把消息挤成逐字竖排。
...
日志通过既有 SSE 控制帧通道推送。SSE 顶层帧以 `kind` 判别，日志走 `kind: "control"` 帧，内层 `payload.control` 为 `"logs"`（复数），与其他控制事件经同一 `discriminatedUnion("control", …)` 区分（`packages/protocol/src/transport/sse-frame.ts:21-54`）。该判别联合当前共九类：`extension-ui` / `queue` / `stats` / `error` / `ui-rpc` / `logs` / `session-status`（就绪握手粘性帧）/ `state`（状态注入桥下行镜像）/ `session-state`（会话权威快照粘性帧）——日志只是其中一支，写 SSE 解析器时应覆盖全集，勿只认前四类。一帧可携带多条 `entries`：
...
- [05 · 分层包](./05-packages.md) — `@blksails/pi-web-logger` 是可独立发布消费的同构日志包（依赖树叶根）
...
- [13 · 配置 UI](./13-config-ui.md) — `logging` 配置域的 schema 驱动设置面与 `logNamespaceToggles` 控件
- [22 · 开发与测试](./22-development-and-testing.md) — 单元测试与 E2E 隔离构建
- [23 · 故障排查 FAQ](./23-troubleshooting-faq.md) — 常见日志相关问题
- [24 · HTTP / SSE API 参考](./24-http-api-reference.md) — `control` 帧全集枚举与 `GET /api/sessions/:id/logs` 历史端点
