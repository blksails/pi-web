...
- `/steer` `/follow_up` `/clear_queue` 端点与 SSE 帧 → [13 HTTP/SSE API 参考](./13-http-api-reference.md)
- 会话就绪握手与 busy 权威快照 → [02 核心概念](./02-core-concepts.md)
- state-injection-bridge(自定义帧接缝的同源范式) → [10 Web UI 扩展](./10-web-ui-extension.md)
- 分层 `@blksails/*` 包职责 → [04 包结构](./04-packages.md)
