...
详见 [02 · 核心概念](./02-core-concepts.md)、[07 · Agent 开发](./07-agent-development.md)。
...
详见 [07 · Agent 开发](./07-agent-development.md)。

### agentHostProvider（规划中 / 未实现）

**规划中的接口隔离点**，意图抽象"如何 spawn agent 子进程"。当前代码中**尚未落地**该工厂——传输接缝由已实现的 `PiRpcChannel`（local 实现 `PiRpcProcess`，本机 `child_process` spawn）承担；`agentHostProvider` 见 `.kiro/steering/roadmap.md` 与 [19 · 路线图](./19-roadmap.md)，是为 docker / e2b / ssh / device 等远程 host 预留的工厂层。详见 [03 · 系统架构](./03-architecture.md)、[19 · 路线图](./19-roadmap.md)。
...
WebExtension 可在 `.pi/web/dist/` 声明一个独立 HTML 表面（`artifact.entry`），宿主以 `<iframe sandbox="allow-scripts">` 加载——不含 `allow-same-origin`，iframe 获得不透明 origin，无法访问宿主 cookie/DOM/凭证。双向通信经 `postMessage`，消息类型由 `@blksails/protocol` 的 `ArtifactMessage` 约束。挂载门控：**必须设置 `NEXT_PUBLIC_PI_EXTENSION_BASE_URL`**，否则 `ArtifactSurface` 不渲染。

详见 [10 · Web UI 扩展](./10-web-ui-extension.md)。
...
`@blksails/agent-kit` 导出的**恒等辅助函数**，运行时原样返回入参，仅为编译期类型推断服务。不使用此包写出的等价 `AgentDefinition` 对象同样能被 runner 载入。
...
详见 [07 · Agent 开发](./07-agent-development.md)、[04 · 包结构](./04-packages.md)。
...
详见 [09 · 扩展与 Skills](./09-extensions-and-skills.md)、[02 · 核心概念](./02-core-concepts.md)。
...
运行时 TypeScript/ESM 加载器。bootstrap runner 通过 `createJiti()` 创建实例，直接在子进程内 import 用户 `index.ts`，无需预编译。jiti 根锚定在 `@blksails/server` 包目录，保证 pi SDK 等依赖从正确位置解析。

详见 [07 · Agent 开发](./07-agent-development.md)、[03 · 系统架构](./03-architecture.md)。
...
`@blksails/protocol` 包导出的**语义化版本字符串**，随每条 SSE 帧携带。客户端可据此检测版本兼容性（`PiProtocolVersionError`）。协议类型/schema 的任何改动都需遵循语义化版本管理。

详见 [04 · 包结构](./04-packages.md)、[13 · HTTP API 参考](./13-http-api-reference.md)。
...
详见 [07 · Agent 开发](./07-agent-development.md)、[02 · 核心概念](./02-core-concepts.md)。
...
活动会话注册表接口（`packages/server/src/session/session-store.ts`），默认实现为 `InMemorySessionStore`——以 `sessionId` 为键的 `Map`，挂在 `globalThis`（`Symbol.for("@blksails/server:InMemorySessionStore")`）以抗 dev 热重载。接口外置，为未来 Redis / Durable Object 等分布式后端预留接缝。
...
WebExtension Tier 1 能力，通过 `defineWebExtension({ slots: { [SlotKey]: ReactNode } })` 向宿主 18 个具名区域（`background`、`headerLeft/Center/Right`、`panelRight`、`empty`、`toolbar`、`statusBar` 等）注入内容。插槽以**追加方式**挂载，不替换宿主内核表面。宿主未声明对应插槽时静默忽略。

详见 [10 · Web UI 扩展](./10-web-ui-extension.md)。

### SSE 帧（Server-Sent Events frame）

前后端通过 `text/event-stream` 协议推送流式数据的单位。每帧携带 `protocolVersion`，event 类型分为：`ui-message-chunk`（消息流）、`control`（控制帧，含 `ui-rpc` 响应、`extension_ui_request`、`stats`）等。浏览器侧 `PiSessionConnection`（`@blksails/react`）负责解析帧并路由到 `ControlStore` 或 `useChat`。

详见 [13 · HTTP API 参考](./13-http-api-reference.md)。
...
详见 [14 · CLI](./14-cli.md)、[15 · 部署](./15-deployment.md)。
...
详见 [03 · 系统架构](./03-architecture.md)、[15 · 部署](./15-deployment.md)。
...
决定一个 agent source 或项目 `.pi/` 目录下资源（skills/extensions/prompts）能否被载入的**可替换策略插件点**。取值 `"always"` 放行、`"ask"`（默认）或 `"never"` 拒绝。cli 模式经 `--approve` / `defaultProjectTrust:"always"` 放行；custom 模式经 `PI_WEB_TRUST_PROJECT` 环境变量传递信任信号。持久化实现：`FsProjectTrustStore`，读写 `<agentDir>/trust.json`（`@blksails/server/trust` 子路径导出）。

详见 [09 · 扩展与 Skills](./09-extensions-and-skills.md)、[03 · 系统架构](./03-architecture.md)。
...
| 术语 | 详述章节 |
|------|---------|
| Agent Source | [02](./02-core-concepts.md)、[07](./07-agent-development.md) |
| AgentDefinition / defineAgent | [07](./07-agent-development.md)、[04](./04-packages.md) |
| agentHostProvider | [03](./03-architecture.md) |
| att\_\<id\> | [08](./08-attachment-system.md) |
| Artifact iframe | [10](./10-web-ui-extension.md) |
| BlobStore / L0–L3 | [08](./08-attachment-system.md) |
| bootstrap runner | [07](./07-agent-development.md) |
| CompletionProvider | [08](./08-attachment-system.md)、[09](./09-extensions-and-skills.md) |
| createPiWebHandler | [03](./03-architecture.md)、[13](./13-http-api-reference.md) |
| 贡献点 | [10](./10-web-ui-extension.md) |
| 双模式载入 | [02](./02-core-concepts.md) |
| extension UI 子协议 | [09](./09-extensions-and-skills.md)、[02](./02-core-concepts.md) |
| formSchema / 表单 IR | [12](./12-config-ui.md) |
| JSONL framing | [02](./02-core-concepts.md)、[03](./03-architecture.md) |
| Kiro steering / spec | [CLAUDE.md](../../CLAUDE.md) |
| openControlOnlyStream | [10](./10-web-ui-extension.md) |
| PiRpcChannel / PiRpcProcess | [02](./02-core-concepts.md)、[03](./03-architecture.md) |
| protocolVersion | [04](./04-packages.md)、[13](./13-http-api-reference.md) |
| renderer（渲染器） | [10](./10-web-ui-extension.md) |
| resolve 投影 | [08](./08-attachment-system.md) |
| runRpcMode | [07](./07-agent-development.md)、[02](./02-core-concepts.md) |
| Session | [02](./02-core-concepts.md)、[03](./03-architecture.md) |
| SessionStore | [03](./03-architecture.md) |
| 插槽（Slots） | [10](./10-web-ui-extension.md) |
| SSE 帧 | [13](./13-http-api-reference.md) |
| standalone | [14](./14-cli.md)、[15](./15-deployment.md) |
| sticky routing | [03](./03-architecture.md)、[15](./15-deployment.md) |
| 声明式布局 | [10](./10-web-ui-extension.md) |
| trustPolicy | [09](./09-extensions-and-skills.md)、[03](./03-architecture.md) |
| WebExtension | [10](./10-web-ui-extension.md) |
...
- 开始开发自定义 agent：[07 · Agent 开发](./07-agent-development.md)。
- 开始编写 WebExtension：[10 · Web UI 扩展](./10-web-ui-extension.md)。
