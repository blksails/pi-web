...
### REQ-20260804-02 · aigc-agent 视频工作室

- Approval evidence:`批准`
- Status:`ACTIVE`
- Version:`1`
- Behavior:`视频工作室并入 examples/aigc-agent；该 Agent 提供隔离 iframe 视频工作室 Pane，支持从创意简报生成镜头草案、逐镜头生成/重试/暂停/继续、实时人工介入、预览、导出及草稿自动保存/恢复。`
- Boundary:`沿用 pi-web 的隔离 Pane 载体与宿主生命周期、TypeScript strict 与既有 Agent/Panes Kit 接口；视频工作室不另设独立 Agent。`
- Acceptance:`(1) `pnpm -C examples/aigc-agent typecheck` exit 0；(2) `pnpm -C examples/aigc-agent build` exit 0 并生成可装载产物；(3) Agent 元数据声明视频工作室 Pane；(4) Pane 首屏可见简报/镜头列表/时间线或预览区域及生成、暂停、重试、人工修改、导出、草稿管理入口；(5) 关键状态迁移、草稿存取与宿主消息契约有自动化测试；(6) 既有 `pnpm typecheck` 与相关 pane/agent 测试不回归。`
- Traceability:`REQ-20260804-02 → examples/aigc-agent/index.ts + video-studio/* + package metadata → tests/e2e evidence。
