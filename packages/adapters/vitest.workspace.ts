/**
 * adapters 包的测试分档。四档定义与判据**与内核包、runner 包、兼容层包完全同形**
 * (spec: adapters-package-extraction,任务 1.2)。
 *
 ...
 *   adapters 与 runner、兼容层一样跨包引用它 —— core 是更低的包,方向正确;反过来会让
 *   core 的测试依赖 adapters。改路径时注意:setup 文件必须用**绝对路径**,相对路径会被
 *   解析到本包根下而静默指空 —— 而没装上的哨兵报出的绿,和真的没有违规长得一模一样。
 */
