...
import { makeStoredEntryNormalizer, parseHeader, parseLine, serializeEntry, serializeHeader } from "@blksails/pi-web-core/session-store/codec.js";
import {
  ...
  type SessionMeta,
  type SessionVersion,
} from "@blksails/pi-web-core/session-store/types.js";
  ...
  async appendBatch(sessionId: string, entries: readonly SessionEntry[]): Promise<void> {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        // 维护去规范化的 name 列(spec auto-session-title, Req 8.4):session_info 即会话显示名,最新生效。
        if (entry.type === "session_info") {
          latestName = (entry as { name?: string }).name;
        }
      }
      if (latestName !== undefined) {
        await client.query("UPDATE sessions SET name = $1 WHERE session_id = $2", [
          latestName,
          sessionId,
        ]);
      }
