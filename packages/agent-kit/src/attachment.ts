 ...
 *
 * `@pi-web/agent-kit` exposes only the **type** contract a server-side tool author
 ...
 * `@pi-web/server` (runner sub-process side) — keeping construction out of
 ...
 * are pure `type` declarations derived from the public `@pi-web/protocol`
 ...
 *
 * 由 `@pi-web/server` 的 `createAttachmentToolContext(store, sessionId)` 构造并经
 * runner 装配注入到 tool;tool 作者经 `@pi-web/agent-kit` 引用本类型。
 *
