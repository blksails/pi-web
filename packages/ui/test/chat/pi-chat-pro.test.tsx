...
describe("PiChatPro 装配", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("渲染富界面:输入区(textarea)、发送按钮、联网开关、附件入口", () => {
    render(<PiChatPro session={mockSession()} controls={mockControls()} />);
  ...
  it("输入文本并点击发送 → transport.sendMessages 被调用并携带文本 (Req 1.2)", async () => {
    ...
    render(<PiChatPro session={session} controls={mockControls()} />);
  ...
  it("带图片附件发送 → sendMessage body.images 经 transport 传入 (Req 3.2)", async () => {
    ...
    const { container } = render(
      <PiChatPro session={session} controls={mockControls()} />,
    );
  ...
  it("会话就绪后调用 controls.getCommands 以填充建议 (Req 10.1)", async () => {
    ...
    render(<PiChatPro session={mockSession()} controls={controls} />);
    await waitFor(() =>
  ...
  it("流式态显示停止按钮,点击触发 controls.abort + useChat stop (Req 2.2/2.3)", async () => {
    ...
    const controls = mockControls();
    render(<PiChatPro session={session} controls={controls} />);
  ...
  it("无 controls 时不报错(优雅降级)", () => {
    expect(() =>
      render(<PiChatPro session={mockSession()} />),
    ).not.toThrow();
  });
});
