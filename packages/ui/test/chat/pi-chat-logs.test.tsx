...
// Mock @pi-web/react so we can capture onLogsFrame callbacks.
let capturedOnLogsFrame: ((entries: import("@pi-web/logger").LogEntry[]) => void) | undefined;
...
function makeControlStore() {
  ...
  return {
    onLogsFrame: vi.fn((cb: (entries: import("@pi-web/logger").LogEntry[]) => void) => {
...
describe("PiChat × LogsPanel 挂载", () => {
  ...
  it("control:logs 帧 → 面板出现对应日志行（实时链路 3.2→面板）", async () => {
    ...
    const entry: import("@pi-web/logger").LogEntry = {
      id: "log-1",
      ts: Date.now(),
      level: "info",
      ns: "agent:test",
