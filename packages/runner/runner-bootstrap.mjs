 ...
 * `@blksails/pi-web-runner` package, so the bare `jiti` import below resolves against
 * the runner package's `node_modules` regardless of where the subprocess is
 * spawned (the agent's working directory, which has no `node_modules`).
 *
 * Responsibilities:
 *  1. Construct a jiti instance rooted at THIS file (the runner package dir) so
 *     jiti + the pi SDK + the TS runner all resolve from the runner package,
...
const runnerPkgDir = dirname(here);
