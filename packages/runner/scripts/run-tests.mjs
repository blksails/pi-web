#!/usr/bin/env node
/**
 * 测试分档编排(spec: test-tiering-fast-lane;runner 包副本由 runner-package-extraction
 * 任务 1.2 建立,照 `../../core/scripts/run-tests.mjs` 同形复制)。
 *
 * ★ 三份都存在是刻意的:每个包各自跑各自的档,合并成一个跨包运行器会让
 ...
 *   `pnpm --filter @blksails/pi-web-runner test -- --run <pattern>` 用法。
 *
 ...
 *
 * runner 提取是分批搬迁的,搬到某一步时可能一个 `.mock.test.ts` 都还没有 ——
 ...
 *   响亮的失败。
 *
 * ⚠ 由此推出:**测试搬入(任务 3.3)之前,本包 fast 档必然为空、按设计就该失败**。
 *   那是本机制在正常工作的证据,不是缺陷;它在 3.3 完成后才应转绿。
 */
