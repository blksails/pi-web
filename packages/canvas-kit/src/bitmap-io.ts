/**
 * bitmap-io — ui `client-image-ops.ts` 原样迁入(task 1.2,Req 5.2;函数语义零变,注释保留)。
 * 唯一结构调整:Annotation / MaskStroke / ExpandEdges 的声明收进 ./types.ts(canonical 家),
 * 本模块转发同一声明 —— 导出清单与原 client-image-ops 逐一对应。
 *
 ...
 */
import type { Annotation, ExpandEdges, MaskStroke } from "./types.js";

export type { Annotation, ExpandEdges, MaskStroke } from "./types.js";
...
// ExpandEdges(四边扩展量)canonical 定义见 ./types.ts。
...
// MaskStroke(一笔掩码笔迹)canonical 定义见 ./types.ts。
...
// ── 标注(线/箭头/文本 → 批注参考图,M2)──────────────────────────────────────────
// Annotation(一条标注)canonical 定义见 ./types.ts。
