 ...
 *  - `buildAigcTools()` 产出 image_generation 与 image_edit 两工具(6.1/6.2);
...
describe("aigc-agent 装配 · 默认路径", () => {
  it("buildAigcTools() 产出 image_generation 与 image_edit 两工具", () => {
  ...
  it("include 筛选只编译子集", () => {
    ...
    expect(tools.map((t) => t.name)).toEqual(["image_generation"]);
  });
  ...
  it("未注入 seam / 缺 provider 密钥 → 工具调用降级而非抛错(5.3)", async () => {
    ...
    try {
      const tools = buildAigcTools();
      const t2i = tools.find((t) => t.name === "image_generation");
