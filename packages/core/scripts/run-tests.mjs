#!/usr/bin/env node
/**
 * 测试分档编排(spec: test-tiering-fast-lane;core 包副本由 core-package-extraction 任务 2.1 建立)。
 *
 * ★ 与 `../../server/scripts/run-tests.mjs` **同形**。两份都存在是刻意的:两个包各自跑各自的档,
 *   合并成一个跨包运行器会让「哪个包红了」变成需要读日志才能回答的问题。
 *
 ...
 *   `pnpm --filter @blksails/pi-web-core test -- --run <pattern>` 用法。
 *
 ...
 *
 * 内核提取是分批搬迁的,core 在搬到某一步时可能一个 `.mock.test.ts` 都还没有 ——
 * vitest 对空档默认 exit 1,那会让「这个包还没有这类测试」和「这个包的测试全炸了」
 * 报出同一个退出码。
 *
 * ⚠ **fast 档故意不给这个标志**。core 的 fast 档装着两个守卫;它一旦变空,
 *   守卫就集体停摆而没有任何人会知道 —— 而这正是本仓已经被骗过两次的失效形态。
 *   fast 档为空必须是一次响亮的失败。
 */
...
if (mode === "--e2e") process.exit(vitest(["--project", "e2e", "--no-file-parallelism", ...ALLOW_EMPTY]));
