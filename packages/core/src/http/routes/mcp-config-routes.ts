...
// ★ 只引**端口**。实现值依赖 MCP SDK,住在兼容层包的 `mcp-probe` 模块,由装配层注入。
//   反过来在这里 `import type` 兼容层包会造成包循环 —— 端口才是正确的解耦方向。
import type { McpProbePort } from "../../config/mcp-probe-port.js";
...
export interface McpConfigRoutesOptions {
  ...
  /** 注入点:便于测试替换探测行为。 */
  /**
   * MCP 探测服务,**必传**(spec: core-package-extraction 任务 4.1)。
   *
   * 原本这里写 `opts.probeService ?? new McpProbeService()` —— 注入缝早就在,只是默认值
   * 把实现拖了进来。改为必传而非"缺省时降级"是刻意的:一个静默不探测的 MCP 配置端点,
   * 表现是"服务器状态永远 unknown",看不出是漏装配还是真探不到。
   */
  readonly probeService: McpProbePort;
}
...
export function createMcpConfigRoutes(opts: McpConfigRoutesOptions): ReadonlyArray<InjectedRoute> {
  ...
  const probeService = opts.probeService;
