 ...
 *    `resolve.alias` 段);**根配置对 `@blksails/pi-web-core` 是配了 alias 的**——但本文件由 `packages/core/vitest.config.ts`(无任何 alias)运行,故仍然到不了那份配置。且根配置的
 *    `include` 只覆盖**根级** `test/`,本文件由 `packages/core/vitest.config.ts`(无任何
 *    alias)运行 —— 故本文件避开 alias 不是靠写法取巧,是**结构上就到不了**那份配置。
 *
 * 因此第 1 层刻意**不**用 `import "@blksails/pi-web-core/testing"` 这种写法,而是直接调用
...
const SUBPATH = "@blksails/pi-web-core/testing";
...
describe("包清单 · 一致性套件子路径导出", () => {
  ...
  it("主入口(`.`)不受影响 —— 新增子路径不得改动既有解析", () => {
    const mainResolved = realpathSync(require_.resolve("@blksails/pi-web-core"));
