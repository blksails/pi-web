 ...
 * - 出口面 = 七组件(Button/Pill/Card/Input/Popover/Select/Textarea 及其子件与
...
import type { ButtonProps, InputProps, PillProps, TextareaProps } from "../src/index.js";
...
describe("@blksails/pi-web-primitives public exports", () => {
  ...
  it("出口纪律:包根值导出=七组件 16 项 + cn,无内部件泄漏(快照)", () => {
  ...
  it("类型导出自包根出口可达(编译期守护;运行时锚定形状抽样)", () => {
    ...
    const pillProps: PillProps = { active: true };
    const textareaProps: TextareaProps = { rows: 3 };
    expect([buttonProps.variant, inputProps.placeholder, pillProps.active, textareaProps.rows]).toEqual([
      "outline",
      "p",
      true,
      3,
