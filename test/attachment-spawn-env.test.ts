...
// other @blksails/server exports real (the handler relies on them).
type CapturedSpec = { env?: Record<string, string>; args: string[] };
const capturedSpecs: CapturedSpec[] = [];
vi.mock("@blksails/server", async () => {
  const actual =
    await vi.importActual<typeof import("@blksails/server")>("@blksails/server");
...
const { attachmentStoreConfigFromEnv } = await import("@blksails/server");
