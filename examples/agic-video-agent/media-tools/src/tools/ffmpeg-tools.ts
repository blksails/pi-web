...
export function registerVideoWithAudio(pi: ExtensionAPI): void {
  registerLocalTool(pi, {
    ...
    parameters: Type.Object({
      ...
      audio_start_seconds: Type.Optional(Type.Number({ minimum: 0, description: "音频在最终视频中的起始位置(秒),默认 0。" })),
      audio_trim_start_seconds: Type.Optional(Type.Number({ minimum: 0, description: "从音频源的哪个位置开始取(秒),默认 0。" })),
      audio_duration_seconds: Type.Optional(Type.Number({ minimum: 0.1, description: "取用音频时长(秒),省略则到音频结束。" })),
      audio_volume: Type.Optional(Type.Number({ minimum: 0, maximum: 2, description: "音量倍率 0–2,默认 1。" })),
    }),
