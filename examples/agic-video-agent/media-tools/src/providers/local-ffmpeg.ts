...
function clampNumber(raw: unknown, min: number, max: number, def: number): number {
  if (typeof raw !== "number" || !Number.isFinite(raw)) return def;
...
export interface VideoWithAudioOptions {
  ...
  readonly audioTrimStartSeconds?: number;
  readonly audioDurationSeconds?: number;
  readonly audioVolume?: number;
}

export function videoWithAudioArgs(
  video: string,
  audio: string,
  output: string,
  mode: "replace" | "mix",
  options: VideoWithAudioOptions = {},
): string[] {
  const audioStart = Math.max(0, options.audioStartSeconds ?? 0);
  const trimStart = Math.max(0, options.audioTrimStartSeconds ?? 0);
  const duration = options.audioDurationSeconds !== undefined && options.audioDurationSeconds > 0
    ? options.audioDurationSeconds
    : undefined;
  const volume = Math.max(0, Math.min(2, options.audioVolume ?? 1));
  ...
  const audioFilter = `[1:a]volume=${volume}${delay > 0 ? `,adelay=${delay}|${delay}` : ""}[audio_track]`;
  if (mode === "mix") {
    return ["-y", "-i", video, ...audioInputOptions, "-i", audio, "-filter_complex", `${audioFilter};[0:a][audio_track]amix=inputs=2:duration=longest:dropout_transition=2[aout]`, "-map", "0:v", "-map", "[aout]", "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-shortest", output];
  }
  return ["-y", "-i", video, ...audioInputOptions, "-i", audio, "-filter_complex", audioFilter, "-map", "0:v", "-map", "[audio_track]", "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-shortest", output];
}
...
export const ffmpegRunLocal = {
  ...
  videoWithAudio: (args: Record<string, unknown>, onStage: OnStage, signal?: AbortSignal): Promise<PickedResult> => {
    ...
    const audioStartSeconds = clampNumber(args["audio_start_seconds"], 0, 3600, 0);
    const audioTrimStartSeconds = clampNumber(args["audio_trim_start_seconds"], 0, 3600, 0);
    const audioDurationSeconds = clampNumber(args["audio_duration_seconds"], 0.1, 3600, 0);
    const audioVolume = clampNumber(args["audio_volume"], 0, 2, 1);
    return runFfmpegTool(
      {
        inputs: { video, audio },
        ...
        kind: "video",
        contentType: "video/mp4",
        caption: `video + audio (${mode}, start=${audioStartSeconds}s, trim=${audioTrimStartSeconds}s${audioDurationSeconds > 0 ? `, duration=${audioDurationSeconds}s` : ""}, volume=${audioVolume})`,
      },
      onStage,
      signal,
    );
