...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log("[4] video_with_audio → 应执行音频裁切/延迟/音量");
    const muxed = await ffmpegRunLocal.videoWithAudio({
      video_url: video,
      audio_url: video,
      mode: "replace",
      audio_trim_start_seconds: 0.2,
      audio_duration_seconds: 0.8,
      audio_start_seconds: 0.3,
      audio_volume: 0.6,
    }, undefined);
    assert(muxed.kind === "video", `kind=video (got ${muxed.kind})`);
    assert("url" in muxed && muxed.url.startsWith("data:video/mp4;base64,"), "带裁切音轨的 data:video/mp4 URI");

    console.log("[5] video_extract_frame → 应得 data:image/png");
    ...
    console.log("[6] persistMedia(audio) → putOutput 收到 audio/mpeg 字节");
    ...
    console.log("[7] runMediaTool + att_ 输入 → 走 localFileFields(att_→localPath),不再报「输入 URL 无效」");
