...
import { materialsPanePackage } from "./packages/materials-pane/src/index.js";
import { searchPanePackage } from "./packages/search-pane/src/index.js";
...
import { videoStudioSurfaceExtension } from "./video-studio/surface.js";
await prefetchPlatformKeys();

// aigc-agent 不展示官方 DashScope;Token Plan 路由另以 token-plan provider 展示并保留。
const AIGC_EXCLUDED_PROVIDERS = new Set(["dashscope"]);

const canvasSurfaceExtension = (pi: ExtensionAPI): void => {
  void makeCanvasSurfaceExtension({ excludedProviders: AIGC_EXCLUDED_PROVIDERS })(pi);
};
...
export default defineAgent({
  ...
  systemPrompt: [
    "你是花影 Agent 视频工作室，擅长把创意简报拆成可执行镜头，并协作完成视频生成。你仍可使用图像工具作为首帧/参考素材：",
    ...
    "- 用户说「搜图/找相似」时，直接调用 `creative_search`；text 与 image_url 二选一，勿仅用文字回答。",
    "",
    ...
    "【视频工作室协作】右侧视频工作室是 agent 权威 Surface。用户要求生成镜头方案时，必须调用 `video_plan`，把 3–6 个结构化镜头写入 Surface；不得用硬编码模板代替 Agent 方案。",
    "用户要求重新生成某镜头提示词时，必须调用 `video_shot_prompt`，将主体、动作、运镜、光线、连续性、负向约束写入指定 shot_id；不得只在对话里输出一段文字。",
    "用户点击某镜头“发送至 Agent 生成”或自动生成时，按镜头顺序处理，不要并发重复调用；必须实际调用合适的视频工具，并把 shot_id 与当前 attachment id 放入上下文。工具完成后报告 attachment id，Surface 会保留同一提示词下的多个视频版本。",
    "用户从历史中选视频或加入视频轨道时，尊重选中的 attachment；用户设置、裁剪、替换或混合音轨后，使用 `video_concat` 与 `video_with_audio` 按轨道生成最终视频。用户暂停或修改时立即尊重最新状态。",
  ].join("\n"),
  ...
  extensions: [
    makeAigcExtension({ excludedProviders: AIGC_EXCLUDED_PROVIDERS }),
    canvasSurfaceExtension,
    aigcPersistExtension,
    mediaToolsExtension,
    videoStudioSurfaceExtension,
    ...searchPanePackage.extensions,
    ...materialsPanePackage.extensions,
  ],
  ...
  // GET /agent-routes/gallery-stats + panes 所需只读 routes；浏览器授权面在 web.config.tsx。
  routes,
