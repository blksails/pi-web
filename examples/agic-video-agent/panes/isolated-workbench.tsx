 ...
 * 复用**同一批** guest 组件(零重写):`SearchApp` / `MaterialsApp` / `CanvasPaneGuest`。
 *
...
import { CanvasPaneGuest } from "@blksails/pi-web-canvas-ui/pane/guest";
import { PaneGuestProvider } from "@blksails/pi-web-panes-kit/react";
import { MaterialsApp } from "../packages/materials-pane/src/guest.js";
import { SearchApp } from "../packages/search-pane/src/guest.js";
import { VideoStudioApp } from "../video-studio/guest.js";
 ...
 *
 * 两份分开:canvas 的 tailwind/ui 样式(canvasCss)只该进 canvas realm —— 与同源形态
 * (search/materials 的 pane-{id}.html 不含 canvasCss)对齐,防样式污染。
 */
declare const __AIGC_PANE_CSS__: string;
declare const __AIGC_CANVAS_CSS__: string;

function injectStyles(paneId: string): void {
...
if (paneRoot !== null) {
  ...
  const paneId =
    (window as unknown as { __PANE_ID__?: string }).__PANE_ID__ ?? "video-studio";
  injectStyles(paneId);
