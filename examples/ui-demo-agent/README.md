...
Zero-config: `@blksails/pi-web-react`'s `useExtensionUI` + `<PiChat extensionUI={…}>` renders all of the above (interactions as inline `<PiInteraction>` cards in the message stream, plus the status bar and notification toasts).
