 ...
 *
 * 「选中工具」作**软引导**:选中即把该工具的 slash(如 `/t2v `)在输入框为空时预填(systemPrompt 已把
 * slash 映射到对应工具);已有输入则只显示胶囊、不改写用户文本。选中亦写会话 KV `aigc.targetedTool`。
 *
 ...
 * 技能面板入口(源项目的「管理技能…」)随 SkillPanel 另轮迁入,此处暂缺。
 */
...
interface ToolDef {
  ...
  /** 选中时预填的 slash(仅输入框为空时);无则只显示意图胶囊。 */
...
/** 把 slash 预填进宿主组合器输入框(受控 textarea:走原生 setter + input 事件)。 */
...
export function AigcPromptToolbar(props: PromptToolbarProps): React.JSX.Element | null {
  ...
  const pickTool = React.useCallback(
    (name: string): void => {
      ...
      if (tool?.slash !== undefined) primeSlash(tool.slash);
      setMenu(null);
    },
    [st],
  );
