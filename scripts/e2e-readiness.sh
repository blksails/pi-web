...
#
# 自动完成:非 standalone 隔离构建 → 起 stub 服务器 → 等就绪 → 外置模式跑 playwright → 清理。
# 规避两个坑:
#   1) `next start` 不兼容 output:standalone → 用 PI_WEB_DISABLE_STANDALONE=1 构建。
#   2) playwright 自管 webServer 在 standalone 配置下超时 → 用外置服务器模式手动起服。
#
# 用法:
...
#   SKIP_BUILD=1 bash scripts/e2e-readiness.sh     # 复用上次 .next-e2e 构建,跳过 build
#
set -euo pipefail
cd "$(dirname "$0")/.."

PORT="${PORT:-3100}"
DIST="${NEXT_DIST_DIR:-.next-e2e}"
SPECS=("session-readiness.e2e.ts")
[[ "${1:-}" == "--all" ]] && SPECS+=("custom-agent.e2e.ts")

# 端口占用预检:避免孤儿 next-server 致 playwright 连到错误/旧构建。
...
cleanup() {
  ...
  # 兜底:杀掉残留在该端口的子进程(next start 会派生 next-server)。
...
if [[ "${SKIP_BUILD:-}" != "1" ]]; then
  echo "▸ 构建(非 standalone, NEXT_DIST_DIR=${DIST})…"
  PI_WEB_DISABLE_STANDALONE=1 NEXT_DIST_DIR="${DIST}" pnpm exec next build >/tmp/e2e-readiness-build.log 2>&1 \
...
SESSION_STORE=fs SESSION_STORE_ROOT="${FS_ROOT}" \
  pnpm exec next start -p "${PORT}" >/tmp/e2e-readiness-server.log 2>&1 &
SERVER_PID=$!
...
# (3) 外置模式跑 playwright。
echo "▸ 运行 e2e: ${SPECS[*]}"
PI_WEB_E2E_EXTERNAL_SERVER=1 PI_WEB_E2E_PORT="${PORT}" NEXT_DIST_DIR="${DIST}" \
